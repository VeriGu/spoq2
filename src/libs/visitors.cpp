    ...
    const NoneConditionAccumulator NoneConditionAccumulator::add_condition(std::unique_ptr<SpecNode> cond) {
        if (accumulated_cond) {
            // if(string(*cond).find("when") != string::npos){
                ...
                // LOG_DEBUG << "last_pattern_match " << (last_pattern_match ? string(*last_pattern_match) : "null");
                // LOG_DEBUG << "...";
            // }
            ...
            /* At this point, we have 1. a current condition in accumulated_cond
             2. A new condition in cond
             3. A possible pointer last_pattern_match to a node in accumulated_cond which cond should be added to.
            
             We will 1. swap the last_pattern_match with a temp symbol, 
                2. copy the accumulated_cond
                3. swap back the original block in accumulated_cond.
                - in new_accumulator.accumulated_cond we now have the original cond with the last_pattern_match
                    swapped with a symbol.
                4. AND the new condition with the temp_held_match_body
                5a. (If or AND) new last_pattern_match is the whole temp_held_match_body
                5b. (Match) new last_pattern_match is the pattern match body in the new match that is `true`
                */
            ...
            if(last_pattern_match){
                if (auto lpm = dynamic_cast<PatternMatch*>(last_pattern_match)) {
                    ...
                    if (const_node == accumulated_cond.get()){
                        accumulated_cond.swap(temp_held_match_body);
                    } else {
            ...
            // LOG_DEBUG << "LPM swapped: " << string(*accumulated_cond);
            ...
            if(last_pattern_match){
                ...
                if (auto lpm = dynamic_cast<PatternMatch*>(last_pattern_match)) {
                    ...
                    auto sym = dynamic_cast<Symbol*>(accumulated_cond.get());
                    if(sym && sym->text == match_body_sym){
                        accumulated_cond = temp_held_match_body->deep_copy();
                        last_pattern_match = accumulated_cond.get();
                    } else {
                        assert(false && "Unexpected condition.");
                    }
                } else {
                    LOG_ERROR << "last_pattern_match's body is neither a PatternMatch nor an AND nor an If expression. This should never happen.";
                    LOG_ERROR << string(*last_pattern_match);
                    assert(false);
                }
            }
            ...
            // If the condition we just added was a match, last_pattern_match currently points
            // to the expression containing the match, rather than the specific pattern_match that we need.
            // We need to find the placed match and get the right PatternMatch node.
            if(is_match){
                Match* match_node = nullptr;
                if(auto and_node = dynamic_cast<Expr*>(new_accumulator.last_pattern_match)){
                    match_node = dynamic_cast<Match*>(and_node->elems->at(0).get());
                    assert(match_node);
                } else if(auto if_node = dynamic_cast<If*>(new_accumulator.last_pattern_match)){
                    match_node = dynamic_cast<Match*>(if_node->cond.get());
                    assert(match_node);
                } else {
                    assert(match_node);
                }
            ...
            if(!temp_free_vars.empty()){
                LOG_ERROR << "Erroneous new free var in add_condition. " << string(*new_accumulator.accumulated_cond);
            }
            
            assert(temp_free_vars.empty());
            assert(new_accumulator.last_pattern_match);
