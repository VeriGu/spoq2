...
void gen_high_proof(Project *p, int i, string fname, string path, string cache_out)
{
    ...
    // Require Import
    std::set<string> deps = {"CommonDeps",
                             "DataTypes",
                             "GlobalDefs",
    ...
    for (auto s : syms) {
        ...
        for (auto d : p->deps[s]) {
            ...
            deps.insert(std::get<0>(p->symbols[d].loc) + "." + std::get<1>(p->symbols[d].loc) + "." +
                        std::get<2>(p->symbols[d].loc));
        }
    }
    ...
    out << "Local Opaque Z.eqb Z.add Z.mul Z.div Z.sub Z.land Z.lor Z.lxor Z.shiftl Z.shiftr Z.quot Z.rem Z.testbit "
           "xorb List.nth.\n";
    out << std::endl;
    ...
    if (mid_syms.size() > 0) {
        if (gen_loop) {
            ...
            out << "           (Hspec: " + fname + "_loop_mid " + join(loop_call_param, " ") + " hst" + " = " +
                       loop_ret("hst'") + "),\n";
            out << "      exists lst', " + fname + "_loop_low " + join(loop_call_param, " ") + " lst" + " = " +
                       loop_ret("lst'") + " /\\ refrel hst' lst'.\n";
            out << "    Proof.\n";
            out << "      intros; inv Hrel.\n";
        ...
        out << "           (Hspec: " + fname + "_spec_mid " + join(call_param, " ") + " hst = " + call_ret("hst'") +
                   "),\n";
        out << "      exists lst', " + fname + "_spec_low " + join(call_param, " ") + " lst = " + call_ret("lst'") +
                   " /\\ refrel hst' lst'.\n";
        out << "    Proof.\n";
        out << "      intros; inv Hrel.\n";
        out << "      autounfold with spec in *; autounfold with sem in *; simpl in *.\n";
        ...
        if (gen_loop) {
            ...
            out << "           (Hrel: refrel hst lst)\n";
            out << "           (Hspec: " + fname + "_loop " + join(loop_call_param, " ") +
                       " hst = " + loop_ret("hst'") + "),\n";
            out << "      exists lst', " + fname + "_loop_mid " + join(loop_call_param, " ") +
                       " lst = " + loop_ret("lst'") + " /\\ refrel hst' lst'.\n";
            out << "    Proof.\n";
            out << "      intros; inv Hrel.\n";
            out << "      autounfold with spec in *; autounfold with sem in *; simpl in *.\n";
            out << "      destruct_spec Hspec; repeat solve_refproof;\n";
        ...
        if (p->has_shadow.find(fname) != p->has_shadow.end()) {
            out << "           (Hspec: " + fname + "_spec_shadow " + join(call_param, " ") +
                       " lst = " + call_ret("hst'") + "),\n";
        } else {
            out << "           (Hspec: " + fname + "_spec " + join(call_param, " ") + " hst = " + call_ret("hst'") +
                       "),\n";
        }
        out << "      exists lst', " + fname + "_spec_mid " + join(call_param, " ") + " lst = " + call_ret("lst'") +
                   " /\\ refrel hst' lst'.\n";
        out << "    Proof.\n";
    ...
    for (auto const &L : p->layers) {
        ...
        for (auto prim : L->prims) {
            ...
            if (fs::exists(ref_proof.string())) {
                files->push_back(
                    (layer_name / boost::filesystem::path(prim) / boost::filesystem::path("RefProof.v")).string());
            }
        }
