...
antlrcpp::Any
ProgramVisitor::visitSection_begin(SpecParser::Section_beginContext *ctx) {
    if (current_layer != nullptr) {
        throw std::runtime_error("Last layer " + current_layer->name +
                                 " not closed");
    } else {
...
antlrcpp::Any
ProgramVisitor::visitSection_end(SpecParser::Section_endContext *ctx) {
    if (current_layer == nullptr) {
        throw std::runtime_error("No layer" + ctx->name()->getText() +
                                 "to close");
    } else {
...
antlrcpp::Any
ProgramVisitor::visitStatement(SpecParser::StatementContext *ctx) {
...
antlrcpp::Any
ProgramVisitor::visitAnno_struct(SpecParser::Anno_structContext *ctx) {
    return std::any();
}
...
antlrcpp::Any
ProgramVisitor::visitGlobal_anno(SpecParser::Global_annoContext *ctx) {
    ctx->name()->getText();
...
antlrcpp::Any ProgramVisitor::visitType(SpecParser::TypeContext *ctx) {
    if (ctx->par) {
        ...
        if (curried_type &&
            ...
            return static_pointer_cast<SpecType>(
                make_shared<Function>(curried_type_func->rettype, args));
        } else {
            return static_pointer_cast<SpecType>(make_shared<Function>(
...
antlrcpp::Any ProgramVisitor::visitDef(SpecParser::DefContext *ctx) {
    ...
    if (name == Project::PROJ_BC_PATH) {
        ...
        while (prims_list && holds_alternative<Expr::binops>(prims_list->op) &&
            ...
            StringConst *prim =
                dynamic_cast<StringConst *>(prims_list->elems->at(0).get());
        ...
        string path =
            string_from_StringConst(expr, "Code path must be a string literal");
        ...
        string load =
            string_from_StringConst(expr, "Load must be a string literal");
        ...
        if (current_layer) {
            if (name.size() > 8 && name.substr(name.size() - 8) == "spec_mid") {
                loc = make_shared<loc_t>(loc_t(current_layer->name,
                                               name.substr(0, name.size() - 9),
                                               Project::LOC_REFPROOF));
            }
            if (name.size() > 8 && name.substr(name.size() - 8) == "spec_low") {
                ...
                loc = make_shared<loc_t>(
                    loc_t(current_layer->name, Project::LOC_SPEC, ""));
            }
        } else {
...
antlrcpp::Any ProgramVisitor::visitFixpoint(SpecParser::FixpointContext *ctx) {
    ...
    proj.add_definition(make_unique<Fixpoint>(name, rettype,
                                              std::move(var_anno),
                                              std::move(expr)),
                        loc);
...
antlrcpp::Any
ProgramVisitor::visitInductive_decl(SpecParser::Inductive_declContext *ctx) {
    string name = ctx->name()->getText();
    auto arms = make_shared<vector<shared_ptr<IndConstr>>>();

    for (auto arm : ctx->induct_arm()) {
        arms->push_back(
            shared_ptr<IndConstr>(any_cast<IndConstr *>(visitInduct_arm(arm))));
    }
...
antlrcpp::Any
ProgramVisitor::visitRecord_fields(SpecParser::Record_fieldsContext *ctx) {
    auto args =
        shared_ptr<vector<shared_ptr<Arg>>>(new vector<shared_ptr<Arg>>());
    auto type = ctx->type();
    auto name = ctx->name();
    auto type_it = type.begin();
    auto name_it = name.begin();

    for (; type_it != type.end() && name_it != name.end();
         type_it++, name_it++) {
        // if ((*name_it)->getText() == "repl") {
        //     LOG_DEBUG << "Visiting record field: " << (*name_it)->getText();
        //     LOG_DEBUG << "type: " << (*type_it)->getText();
        //     LOG_DEBUG << "Parsed type: " <<
        //     string(*any_cast<shared_ptr<SpecType>>(visitType(*type_it)));
        // }
        args->push_back(make_shared<Arg>(
            (*name_it)->getText(),
            any_cast<shared_ptr<SpecType>>(visitType(*type_it))));
    }
...
antlrcpp::Any ProgramVisitor::visitExpr(SpecParser::ExprContext *ctx) {
    if (ctx->if_stmt()) {
        ...
        return any_cast<SpecNode *>(
            visitRecord_def_stmt(ctx->record_def_stmt()));
    }
...
antlrcpp::Any
ProgramVisitor::visitRecord_def_stmt(SpecParser::Record_def_stmtContext *ctx) {
    auto raw_fields = visitRecord_fields_def(ctx->record_fields_def());
    auto fields =
        unique_ptr<std::map<unique_ptr<Symbol>, unique_ptr<SpecNode>>>(
            any_cast<std::map<unique_ptr<Symbol>, unique_ptr<SpecNode>> *>(
                raw_fields));
...
antlrcpp::Any ProgramVisitor::visitRecord_fields_def(
    SpecParser::Record_fields_defContext *ctx) {
    ...
    for (auto it = names.begin(); it != names.end(); it++) {
        ...
        auto value =
            unique_ptr<SpecNode>(any_cast<SpecNode *>(visitName(*(++it))));
...
antlrcpp::Any ProgramVisitor::visitExpr_op(SpecParser::Expr_opContext *ctx) {
    if (ctx->binop) {
        ...
        auto op0 = unique_ptr<SpecNode>(
            any_cast<SpecNode *>(visitExpr_op(ctx->expr_op(0))));
        auto name =
            unique_ptr<SpecNode>(any_cast<SpecNode *>(visitName(ctx->name(0))));
        auto op1 = unique_ptr<SpecNode>(
            any_cast<SpecNode *>(visitExpr_op(ctx->expr_op(1))));
        auto elems = make_unique<vector<unique_ptr<SpecNode>>>();
        ...
        auto vars = unique_ptr<vector<shared_ptr<Arg>>>(
            any_cast<vector<shared_ptr<Arg>> *>(
                visitForall_expr(ctx->forall_expr())));
        auto op0 = unique_ptr<SpecNode>(
            any_cast<SpecNode *>(visitExpr_op(ctx->expr_op(0))));
        ...
        auto vars = unique_ptr<vector<shared_ptr<Arg>>>(
            any_cast<vector<shared_ptr<Arg>> *>(
                visitExists_expr(ctx->exists_expr())));
        auto op0 = unique_ptr<SpecNode>(
            any_cast<SpecNode *>(visitExpr_op(ctx->expr_op(0))));
        ...
        for (auto it = term.begin() + 1; it != term.end(); it++) {
            elems->push_back(
                unique_ptr<SpecNode>(any_cast<SpecNode *>(visitTerm(*it))));
        }

        if (dynamic_cast<Symbol *>(op.get()) != nullptr) {
            // static const unordered_map<string, Expr::binops>
            // str_to_binops_map = {
            //     {"Z.lnot", Expr::Zlnot},
            //     {"Z.lxor", Expr::Zlxor},
            ...
            // if (str_to_binops_map.find(((Symbol *)op.get())->text) !=
            // str_to_binops_map.end()) {
            //     return (SpecNode *)(new Expr(str_to_binops_map.at(((Symbol
            //     *)op.get())->text), std::move(elems)));
            // } else
...
antlrcpp::Any ProgramVisitor::visitTerm(SpecParser::TermContext *ctx) {
    if (ctx->uniop) {
        ...
        if (op == Expr::ops::__NEG)
            return (
                SpecNode *)(new Expr(Expr::binops::MINUS, std::move(elems)));

        return (
            SpecNode *)(new Expr(parse_uniop(ctx->uniop), std::move(elems)));
...
antlrcpp::Any ProgramVisitor::visitLet_stmt(SpecParser::Let_stmtContext *ctx) {
    auto src =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr(0))));
    auto body =
...
antlrcpp::Any
ProgramVisitor::visitMatch_stmt(SpecParser::Match_stmtContext *ctx) {
    auto src =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr())));
    auto match_list = make_unique<vector<unique_ptr<PatternMatch>>>();

    for (auto match_branch : ctx->match_branch()) {
        match_list->push_back(unique_ptr<PatternMatch>(
...
antlrcpp::Any
ProgramVisitor::visitMatch_branch(SpecParser::Match_branchContext *ctx) {
    auto pattern =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr(0))));
    auto body =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr(1))));
    ...
    if (dynamic_cast<Symbol *>(pattern.get())) {
        ...
        if (s->text == "None") {
            pattern = unique_ptr<SpecNode>(new Expr(
                Expr::None, make_unique<vector<unique_ptr<SpecNode>>>()));
        }
    }
...
antlrcpp::Any
ProgramVisitor::visitWhen_stmt(SpecParser::When_stmtContext *ctx) {
...
antlrcpp::Any
ProgramVisitor::visitAssert_stmt(SpecParser::Assert_stmtContext *ctx) {
    auto prop =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr(0))));
    auto body =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr(1))));
...
antlrcpp::Any
ProgramVisitor::visitAnno_stmt(SpecParser::Anno_stmtContext *ctx) {
    auto prop =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr(0))));
    auto body =
        unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(ctx->expr(1))));
...
antlrcpp::Any
ProgramVisitor::visitFunc_call(SpecParser::Func_callContext *ctx) {
    return std::any();
}

// Returns a vector<shared_ptr<Arg>>*
antlrcpp::Any
ProgramVisitor::visitForall_expr(SpecParser::Forall_exprContext *ctx) {
...
antlrcpp::Any
ProgramVisitor::visitExists_expr(SpecParser::Exists_exprContext *ctx) {
    auto args = new vector<shared_ptr<Arg>>();
...
antlrcpp::Any ProgramVisitor::visitIf_stmt(SpecParser::If_stmtContext *ctx) {
    ...
    return (
        SpecNode *)(new If(std::move(cond), std::move(then), std::move(els)));
}
...
antlrcpp::Any ProgramVisitor::visitTuple(SpecParser::TupleContext *ctx) {
    ...
    for (auto expr : ctx->expr()) {
        elems->push_back(
            unique_ptr<SpecNode>(any_cast<SpecNode *>(visitExpr(expr))));
    }
