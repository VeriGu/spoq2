...
namespace autov {
    ...
    bool check_states_implies_pre_condition(Project* proj, const shared_ptr<ProveState>& state, const string& fname, vector<unique_ptr<SpecNode>>* elems);
    z3::expr formulate_post_cond_z3(Project* proj, const std::string& fname, SpecNode* func_call, const shared_ptr<ProveState>& state);
