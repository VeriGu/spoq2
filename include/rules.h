...
/// Try to match [pattern] against [src], filling [assigns] with the bindings the
/// pattern would introduce.  Returns [def] when it cannot tell -- and on those
/// paths [assigns] is left incomplete, so a caller that substitutes the bindings
/// must pass false.
