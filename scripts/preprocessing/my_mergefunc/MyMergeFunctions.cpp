...
static cl::opt<bool>
    MergeFunctionsPDI("mymergefunc-preserve-debug-info", cl::Hidden,
                      cl::init(false),
                      cl::desc("Preserve debug info in thunk when mergefunc "
                               "transformations are made."));

static cl::opt<bool>
    MergeFunctionsAliases("mymergefunc-use-aliases", cl::Hidden,
                          cl::init(false),
...
private:
  ...
  public:
    ...
    bool operator()(const FunctionNode &LHS, const FunctionNode &RHS) const {
      std::string lname = LHS.getFunc()->getName().str();
      std::string rname = RHS.getFunc()->getName().str();
      size_t dotPosL = lname.find(".");
      size_t dotPosR = rname.find(".");
      std::string fprefix = lname.substr(0, dotPosL);
      std::string gprefix = rname.substr(0, dotPosR);
      // Order first by hashes, then full function comparison.
      if (LHS.getHash() != RHS.getHash()) {
        if (fprefix == gprefix)
          LLVM_DEBUG(errs() << "(my merge func): prefixes are equal: " << fprefix << " " << gprefix << "\n");
        return LHS.getHash() < RHS.getHash();
      }
      FunctionComparator FCmp(LHS.getFunc(), RHS.getFunc(), GlobalNumbers);
      bool ret = FCmp.compare() == -1;
      if (fprefix == gprefix && FCmp.compare())
        LLVM_DEBUG(errs() << "(my merge func): equal prefix w. diff body: " << lname << " " << rname << "\n");
      else if (fprefix == gprefix)
        LLVM_DEBUG(errs() << "(my merge func): equal func: " << lname << " " << rname << "\n");

      if (fprefix != gprefix)
        return fprefix < gprefix;
      return ret;
    }
  };
...
// INITIALIZE_PASS(MergeFunctionsLegacyPass, "mymergefunc",
...
// static cl::opt<bool>
//     MergeFunctionsCMD("mymergefunc", cl::Hidden,
//                       cl::init(false),
//                       cl::desc("My merge functions pass"));
...
bool MergeFunctions::doSanityCheck(std::vector<WeakTrackingVH> &Worklist) {
  if (const unsigned Max = NumFunctionsForSanityCheck) {
    ...
    LLVM_DEBUG(dbgs() << "MERGEFUNC-SANITY: Started for first " << Max << " functions.\n");
    ...
    for (std::vector<WeakTrackingVH>::iterator I = Worklist.begin(),
      ...
      for (std::vector<WeakTrackingVH>::iterator J = I; J != E && j < Max;
        ...
        if (Res1 != -Res2) {
          LLVM_DEBUG(dbgs() << "MERGEFUNC-SANITY: Non-symmetric; triple: " << TripleNumber
                 << "\n");
          LLVM_DEBUG(dbgs() << *F1 << '\n' << *F2 << '\n');
          Valid = false;
        }
        ...
        for (std::vector<WeakTrackingVH>::iterator K = J; K != E && k < Max;
          ...
          if (!Transitive) {
            LLVM_DEBUG(dbgs() << "MERGEFUNC-SANITY: Non-transitive; triple: "
                   << TripleNumber << "\n");
            LLVM_DEBUG(dbgs() << "Res1, Res3, Res4: " << Res1 << ", " << Res3 << ", "
                   << Res4 << "\n");
            LLVM_DEBUG(dbgs() << *F1 << '\n' << *F2 << '\n' << *F3 << '\n');
            Valid = false;
          }
    ...
    LLVM_DEBUG(dbgs() << "MERGEFUNC-SANITY: " << (Valid ? "Passed." : "Failed.") << "\n");
    return Valid;
  }
  return true;
}
...
bool MergeFunctions::writeThunkOrAlias(Function *F, Function *G) {
  ...
  LLVM_DEBUG(errs() << "writeThunkOrAlias: " << F->getName() << " " << G->getName() << "\n");
...
void MergeFunctions::mergeTwoFunctions(Function *F, Function *G) {
  std::string fname = F->getName().str();
  std::string gname = G->getName().str();
  size_t dotPosf = fname.find(".");
  size_t dotPosg = gname.find(".");
  // If neither function has a naming conflict, we do not want to merge them.
  if (dotPosf == std::string::npos && dotPosg == std::string::npos) {
      LLVM_DEBUG(errs() << "(my merge func): No mergeTwoFunctions " << fname <<
                " and " << gname << "\n");
      return;
  } else {
    std::string fprefix = fname.substr(0, dotPosf);
    std::string gprefix = gname.substr(0, dotPosg);
    ...
    LLVM_DEBUG(errs() << "(my merge func): mergeTwoFunctions " << fname << \
              " and " << gname << "\n");
  }
  if (F->isInterposable()) {
    ...
    if (!G->isInterposable() && !MergeFunctionsPDI) {
        LLVM_DEBUG(errs() << "G is not Inter: " << F->getName() << " " << G->getName() << "\n");

      if (G->hasGlobalUnnamedAddr()) {
        LLVM_DEBUG(errs() << "Merge Two " << __LINE__ << " "<< F->getName() << " " << G->getName() << "\n");
...
static bool isFuncOrderCorrect(const Function *F, const Function *G) {
  LLVM_DEBUG(errs() << "compare: " << F->getName().str() << " and " << G->getName().str()
                      << '\n');
...
bool MergeFunctions::insert(Function *NewFunction) {
  ...
  if (Result.second) {
    ...
    LLVM_DEBUG(errs() << "Inserting as unique: " << NewFunction->getName()
                      << '\n');
    return false;
  }
  ...
  std::string fname = OldF.getFunc()->getName().str();
  std::string gname = DeleteF->getName().str();
...
// New PM Registration
//-----------------------------------------------------------------------------
// llvm::PassPluginLibraryInfo getMyMergeFuncPluginInfo() {
//   return {LLVM_PLUGIN_API_VERSION, "MyMergeFunc", LLVM_VERSION_STRING,
//           [](PassBuilder &PB) {
//             PB.registerPipelineParsingCallback(
//                 [](StringRef Name, ModulePassManager &MPM,
//                    ArrayRef<PassBuilder::PipelineElement>) {
//                   if (Name == "my-mergefunc") {
//                     MPM.addPass(MergeFunctions());
//                     return true;
//                   }
//                   return false;
//                 });
...
//   return getMyMergeFuncPluginInfo();
// }
