...
Record Layer :=
  mkLayer {
    ...
    Free: string -> State -> State -> option State;
...
Class IntPtrCast :=
  {
    ...
    ptr_int_same:
        forall p, 0 <= p.(poffset) < size_of_base (p.(pbase)) -> int_to_ptr (ptr_to_int p) = p
  }.
...
Section Semantics.
  ...
  Notation ptr_to_int := L.(Ptr2Int).
  Notation int_to_ptr := L.(Int2Ptr).
  Notation ptr_eqb := L.(PtrEqb).
  Notation ptr_gtb := L.(PtrGtb).
  Notation ptr_ltb := L.(PtrLtb).
  ...
  Definition fit_val_typ (t: typ) (v: value) : option value :=
    ...
    | TBool, VInt v => Some (VBool (negb (v =? 0)))
    | _, _ => Some v
    end.
  ...
  Fixpoint eval (te: temp_env) (val: value) {struct val} : option value := 
    ...
    | VGlobal name =>
      ...
      | _ => match gets name md.(functions) with
             | Some fname => Some (VPtr (mkPtr name 0))
             | _ => None
             end
      end
    ...
    | VExpr ty op vals =>
      ...
      | Some vals' =>
        ...
        | Cslt, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_ltb va vb))
        | Csle, [VInt va; VInt vb] => fit_val_typ ty (VBool (va <=? vb))
        | Csle, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_ltb va vb || ptr_eqb va vb))
        | Cult, [VInt va; VInt vb] => fit_val_typ ty (VBool (va <? vb))
        | Cult, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_ltb va vb))
        | Cule, [VInt va; VInt vb] => fit_val_typ ty (VBool (va <=? vb))
        | Cule, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_ltb va vb || ptr_eqb va vb))
        ...
        | Csgt, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_gtb va vb))
        | Csge, [VInt va; VInt vb] => fit_val_typ ty (VBool (va >=? vb))
        | Csge, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_gtb va vb || ptr_eqb va vb))
        | Cugt, [VInt va; VInt vb] => fit_val_typ ty (VBool (va >? vb))
        | Cugt, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_gtb va vb))
        | Cuge, [VInt va; VInt vb] => fit_val_typ ty (VBool (va >=? vb))
        | Cuge, [VPtr va; VPtr vb] => fit_val_typ ty (VBool (ptr_gtb va vb || ptr_eqb va vb))
  ...
  | exec_alloca:
    forall fname ty assign align te st ptr te' st',
      alloca fname (load_store_typ ty) align st = Some (ptr, st') ->
      sets assign (VPtr ptr) te = te' ->
      exec (IAlloca fname (TPtr ty) assign align :: nil) te st te' out_normal st'
  | exec_assign:
  ...
  | exec_call_void_local:
    forall fname ptr args arglist te st st',
    eval_list te args = Some arglist ->
    eval te ptr = Some (VPtr (mkPtr fname 0)) ->
    exec_func fname arglist st st' None ->
    exec (ICall TVoid None ptr args :: nil) te st te out_normal st'
  | exec_call_ret_local:
    forall fname ty assign ptr args arglist te st val te' st',
    eval_list te args = Some arglist ->
    eval te ptr = Some (VPtr (mkPtr fname 0)) ->
    exec_func fname arglist st st' (Some val) ->
    sets assign val te = te' ->
    exec (ICall ty (Some assign) ptr args :: nil) te st te' out_normal st'
  | exec_list:
  ...
  | exec_func_call:
    forall fname func arglist body te st0 st st' te' ret st'',
      gets fname funcs = Some func ->
      new_frame fname st0 = Some st ->
      init_temp_env func.(fargs) arglist nil = Some te ->
      func.(fbody) = Some body ->
      exec body te st te' (out_return ret) st' ->
      free fname st0 st' = Some st'' ->
      exec_func fname arglist st0 st'' ret.
  (*
