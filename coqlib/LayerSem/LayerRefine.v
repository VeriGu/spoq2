...
Section LayerRefine.
  ...
  Record LayerRef (L H: Layer) (M: module) (rel: L.(State) -> H.(State) -> Prop) : Prop :=
    mkLayerRef {
      ...
      alloca_ref:
        forall fn t a p st_l st_h st_h',
          rel st_l st_h ->
          H.(Alloca) fn t a st_h = Some (p, st_h') ->
          exists st_l', 
            L.(Alloca) fn t a st_l = Some (p, st_l') /\ rel st_l' st_h';
      free_ref:
        forall fn st_l st_l_init st_h_init st_h st_h',
          rel st_l st_h ->
          rel st_l_init st_h_init ->
          H.(Free) fn st_h_init st_h = Some st_h' ->
          exists st_l',
            L.(Free) fn st_l_init st_l = Some st_l' /\ rel st_l' st_h';
      prim_ref:
        forall name,
