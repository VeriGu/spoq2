...
namespace ir2json
{
    ...
    json::Value gen_BranchInst(llvm::BranchInst &I)
    {
        ...
        llvm:Instruction *orig_inst = llvm::dyn_cast<llvm::Instruction>(&I);
        const llvm::DebugLoc &debugInfo = orig_inst->getDebugLoc();

        inst["type"] = "BranchInst";

        if (debugInfo && debugInfo->getLine() != 0)
                inst["lineno"] = debugInfo->getLine();
    ...
    json::Value gen_CallBrInst(llvm::CallBrInst &I)
    {
        ...
        inst["assign"] = gen_value(&I);
        inst["default"] = gen_value(I.getDefaultDest());
        inst["indirect"] = json::Array();
        for (int i = 0; i < I.getNumIndirectDests(); ++i)
        {
            inst["indirect"].getAsArray()->push_back(gen_value(I.getIndirectDest(i)));
        }
        return inst;
    }
    ...
    json::Value gen_instruction(llvm::Instruction &inst)
    {
        ...
        if (llvm::CallBrInst *I = dyn_cast<llvm::CallBrInst>(&inst))
            return gen_CallBrInst(*I);
