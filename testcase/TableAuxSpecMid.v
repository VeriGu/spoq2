Definition __find_lock_next_level_spec_mid (v_g_tbl: Ptr) (v_map_addr: Z) (v_level: Z) (st: RData) : (option (Ptr * RData)) :=
  ...
  then (
    ...
    (anno ((((16 * ((((ret & (281474976710655)) & ((- 4096))) / (GRANULE_SIZE)))) / (ST_GRANULE_SIZE)) = ((((ret & (281474976710655)) & ((- 4096))) / (GRANULE_SIZE)))));
    ...
    (anno (((- 1) = ((- 1))));
    (anno (((0 - (0)) = (0)));
    (Some ((mkPtr "null" 0), (st.[share].[slots] :< (((st.(share)).(slots)) # SLOT_RTT == (- 1))))))))))).

Definition validate_data_create_spec_mid (v_map_addr: Z) (v_rd: Ptr) (st: RData) : (option (Z * RData)) :=
