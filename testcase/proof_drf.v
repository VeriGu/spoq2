Definition PROJ_NAME: string := "RMMProof.ProofGenDRF".
Definition PROJ_BASE: string := "./ProofGenDRF".
Hint CacheSpec.
...
(* Obsolete *)
Definition __s_granule_field_accessible (g: s_granule) (ofs: Z) : bool :=
...
(* Only used by load/store_RData for non-atomic operations *)
Definition s_granule_field_accessible (g: s_granule) (ofs: Z) : Prop :=
  g.(e_lock) = Some CPU_ID.
Parameter sysreg_handlers_base : Z.

Parameter load_store_oracle: RData -> (option (Z * RData)).
Hint Axiom (forall (i: Z) (ret: Z) (st1: RData) (st2: RData) ,
      ((load_store_oracle st1 = Some (ret, st2)) ->
       (((st1.(share).(granules) @ i) = (st2.(share).(granules) @ i))))).
Hint Axiom (forall  (i: Z) (ret: Z) (st1: RData) (st2: RData),
      ((load_store_oracle st1 = Some (ret, st2)) ->
       (((st1.(share).(granules) @ i).(e_lock) = (st2.(share).(granules) @ i).(e_lock))))).
Hint Axiom (forall (i: Z) (ret: Z) (st1: RData) (st2: RData) ,
      ((load_store_oracle st1 = Some (ret, st2)) ->
       (((st1.(share).(granules) @ i).(e_refcount) = (st2.(share).(granules) @ i).(e_refcount))))).
Hint Axiom (forall (i: Z) (ret: Z) (st1: RData) (st2: RData),
      ((load_store_oracle st1 = Some (ret, st2)) ->
      ((st1.(share).(slots) @ i) = (st2.(share).(slots) @ i)))).


Parameter load_oracle: RData -> (option Z).
Definition load_RData (sz: Z) (p: Ptr) (st: RData) : (option (Z * RData)) :=
  if p.(pbase) =s "granules" then (
      ...
      rely (s_granule_field_accessible (ZMap.get st.(share).(granules) idx) ofs);
      (* Lock is not to be loaded directly *)
      if ofs =? 0 then None else
      if (ofs =? 8) && ((ZMap.get st.(share).(granules) idx).(e_state) <>? GRANULE_STATE_RTT) then
        None
      else
        when v == (load_oracle st); Some (v, st) ) else
  ...
  if p.(pbase) =s "slot_rec" then (
      ...
      when v == (load_oracle st); Some (v, st)
    ) else
  ...
  if p.(pbase) =s "slot_rtt" then (
      ...
      let g := st.(share).(granules) @ g_idx in
      match g.(e_lock) with
      | Some cid => when v == (load_oracle st); Some (v, st)
      | None => None
      end ) else
...
(* Parameter store_RData_oracle: RData -> (option RData). *)
(* Hint Axiom (forall (i: Z) (st1: RData) (st2: RData) , *)
(*       ((store_RData_oracle st1 = Some st2) -> *)
(*        (((st1.(share).(granules) @ i) = (st2.(share).(granules) @ i))))). *)
(* Hint Axiom (forall  (i: Z) (st1: RData) (st2: RData), *)
(*       ((store_RData_oracle st1 = Some st2) -> *)
(*        (((st1.(share).(granules) @ i).(e_lock) = (st2.(share).(granules) @ i).(e_lock))))). *)
...
Definition store_RData (sz: Z) (p: Ptr) (v: Z) (st: RData) : option RData :=
  if p.(pbase) =s "granules" then let ofs := p.(poffset) in (
      ...
      rely (s_granule_field_accessible (ZMap.get st.(share).(granules) idx) elem_ofs);
      when _v, st == load_store_oracle st;
      if elem_ofs =? 0 then None else
      (* refcount *)
      if (elem_ofs =? 8) && ((ZMap.get st.(share).(granules) idx).(e_state) <>? GRANULE_STATE_RTT) then
        None
      else (
        if elem_ofs =? 8 then
          let ret := ((ZMap.get st.(share).(granules) idx).[e_refcount] :< v) in
  ...
  if p.(pbase) =s "slot_rec2" then (
      ...
      rely (rec_field_accessible g locked st = true);
      when _v, st == load_store_oracle st; Some st
    ) else
  if p.(pbase) =s "slot_rtt" then let ofs := p.(poffset) in
      ...
      let g := st.(share).(granules) @ g_idx in
      match g.(e_lock) with
      | Some cid => when _v, st == load_store_oracle st; Some st
      | None => None
      end else
  if p.(pbase) =s "slot_rec_aux0" then (
      let ofs := p.(poffset) in
...
Definition base_to_slot (b: string) : option Z :=
  ...
  if b =s "slot_rsi_call" then Some SLOT_RSI_CALL else
  (* Generated base_to_slot begins *)
  if b =s "attest_setup_platform_token_stack" then (Some STACK_attest_setup_platform_token) else
  if b =s "smc_psci_complete_stack" then (Some STACK_smc_psci_complete) else
  if b =s "find_lock_two_granules_stack" then (Some STACK_find_lock_two_granules) else
  if b =s "attest_token_continue_write_state_stack" then (Some STACK_attest_token_continue_write_state) else
  if b =s "rmm_log_stack" then (Some STACK_rmm_log) else
  if b =s "attest_realm_token_create_stack" then (Some STACK_attest_realm_token_create) else
  if b =s "smc_rec_enter_stack" then (Some STACK_smc_rec_enter) else
  if b =s "do_host_call_stack" then (Some STACK_do_host_call) else
  if b =s "attest_rnd_prng_init_stack" then (Some STACK_attest_rnd_prng_init) else
  if b =s "plat_setup_stack" then (Some STACK_plat_setup) else
  if b =s "attest_token_encode_start_stack" then (Some STACK_attest_token_encode_start) else
  if b =s "smc_data_destroy_stack" then (Some STACK_smc_data_destroy) else
  if b =s "xlat_get_llt_from_va_stack" then (Some STACK_xlat_get_llt_from_va) else
  if b =s "smc_rec_create_stack" then (Some STACK_smc_rec_create) else
  if b =s "measurement_extend_sha512_stack" then (Some STACK_measurement_extend_sha512) else
  if b =s "data_granule_measure_stack" then (Some STACK_data_granule_measure) else
  if b =s "sort_granules_stack" then (Some STACK_sort_granules) else
  if b =s "measurement_extend_sha256_stack" then (Some STACK_measurement_extend_sha256) else
  if b =s "realm_ipa_to_pa_stack" then (Some STACK_realm_ipa_to_pa) else
  if b =s "attest_realm_token_sign_stack" then (Some STACK_attest_realm_token_sign) else
  if b =s "rmm_el3_ifc_get_platform_token_stack" then (Some STACK_rmm_el3_ifc_get_platform_token) else
  if b =s "attest_init_realm_attestation_key_stack" then (Some STACK_attest_init_realm_attestation_key) else
  if b =s "plat_cmn_setup_stack" then (Some STACK_plat_cmn_setup) else
  if b =s "complete_rsi_host_call_stack" then (Some STACK_complete_rsi_host_call) else
  if b =s "handle_realm_rsi_stack" then (Some STACK_handle_realm_rsi) else
  if b =s "smc_rtt_set_ripas_stack" then (Some STACK_smc_rtt_set_ripas) else
  if b =s "rtt_walk_lock_unlock_stack" then (Some STACK_rtt_walk_lock_unlock) else
  if b =s "smc_rtt_destroy_stack" then (Some STACK_smc_rtt_destroy) else
  if b =s "map_unmap_ns_stack" then (Some STACK_map_unmap_ns) else
  if b =s "handle_rsi_attest_token_init_stack" then (Some STACK_handle_rsi_attest_token_init) else
  if b =s "realm_params_measure_stack" then (Some STACK_realm_params_measure) else
  if b =s "handle_rsi_ipa_state_get_stack" then (Some STACK_handle_rsi_ipa_state_get) else
  if b =s "realm_ipa_get_ripas_stack" then (Some STACK_realm_ipa_get_ripas) else
  if b =s "smc_rtt_fold_stack" then (Some STACK_smc_rtt_fold) else
  if b =s "smc_rtt_create_stack" then (Some STACK_smc_rtt_create) else
  if b =s "rsi_log_on_exit_stack" then (Some STACK_rsi_log_on_exit) else
  if b =s "attest_cca_token_create_stack" then (Some STACK_attest_cca_token_create) else
  if b =s "rec_params_measure_stack" then (Some STACK_rec_params_measure) else
  if b =s "handle_ns_smc_stack" then (Some STACK_handle_ns_smc) else
  if b =s "rmm_el3_ifc_get_realm_attest_key_stack" then (Some STACK_rmm_el3_ifc_get_realm_attest_key) else
  if b =s "handle_rsi_realm_config_stack" then (Some STACK_handle_rsi_realm_config) else
  if b =s "smc_rtt_init_ripas_stack" then (Some STACK_smc_rtt_init_ripas) else
  if b =s "smc_rtt_read_entry_stack" then (Some STACK_smc_rtt_read_entry) else
  if b =s "handle_data_abort_stack" then (Some STACK_handle_data_abort) else
  if b =s "data_create_stack" then (Some STACK_data_create) else
  if b =s "smc_realm_create_stack" then (Some STACK_smc_realm_create) else
  if b =s "ripas_granule_measure_stack" then (Some STACK_ripas_granule_measure) else
  if b =s "ipa_is_empty_stack" then (Some STACK_ipa_is_empty) else
...
Section Bottom.
  ...
  (* memcpy *)
  Parameter llvm_memcpy_p0i8_p0i8_i64_spec_state_oracle: Ptr -> (Ptr -> (Z -> (bool -> (RData -> (option RData))))).
  Definition llvm_memcpy_p0i8_p0i8_i64_spec (v_dest: Ptr) (v_src: Ptr) (sz: Z) (is_volatile: bool) (st: RData) : option RData :=
    llvm_memcpy_p0i8_p0i8_i64_spec_state_oracle v_dest v_src sz is_volatile st.
...
Section Helpers.
  ...
  Hint InitRely __granule_refcount_dec ((((((st.(share)).(granules)) @ (((v_g.(poffset)) + (8)) / (ST_GRANULE_SIZE))).(e_state)) - (GRANULE_STATE_RTT)) = (0)).
...
Section InjectAbort.
  ...
  Hint NoTrans realm_inject_undef_abort_spec_mid.
  Hint NoTrans inject_sync_idabort_spec_mid.
  Hint NoTrans inject_serror_spec_mid.
End InjectAbort.
...
(*   Hint InitRely simd_save_state (((v_simd.(pbase) = "slot_rec_aux0") /\ (REC_HEAP_SIZE + REC_PMU_SIZE = v_simd.(poffset))) \/ (v_simd.(pbase) = "g_ns_simd")). *)
(*   Hint InitRely simd_restore_state (((v_simd.(pbase) = "slot_rec_aux0") /\ (REC_HEAP_SIZE + REC_PMU_SIZE = v_simd.(poffset))) \/ (v_simd.(pbase) = "g_ns_simd")). *)
...
(*   Hint InitRely rec_simd_enable_restore (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
...
(*     2 - v_i_143. *)
(*   Hint InitRely find_lock_granules_loop0_low (v_granules.(pbase) = "find_lock_two_granules_stack"). *)
(*   Hint InitRely find_lock_granules_loop197_low (v_granules.(pbase) = "find_lock_two_granules_stack"). *)
(*   Hint InitRely find_lock_granules_loop0_mid (v_granules.(pbase) = "find_lock_two_granules_stack"). *)
(*   Hint InitRely find_lock_granules_loop197_mid (v_granules.(pbase) = "find_lock_two_granules_stack"). *)
...
(*   Hint NoUnfold find_lock_two_granules_spec_mid. *)

(*   Include "./LockGranulesSpecMid.v". *)
...
(*   (* Hint NoUnfold get_rd_rec_count_unlocked_spec. *) *)
(*   (* Hint NoUnfold get_rd_rec_count_unlocked_spec_mid. *) *)

(*   Include "./RecInfoSpecMid.v". *)
...
(*   Include "ValidateTableSpecMid.v". *)
...
(*       nil. *)
(*   Include "TableAuxSpecMid.v". *)
(*   Include "TableAuxSpec.v". *)
(* End TableAux. *)
...
(*     v_level. *)
(*   Hint NoTrans rtt_walk_lock_unlock_spec_mid. *)
(*   Hint NoTrans rtt_walk_lock_unlock_loop370_mid. *)
(*   Hint NoUnfold rtt_walk_lock_unlock_spec. *)
(*   Hint NoUnfold rtt_walk_lock_unlock_spec_mid. *)
(*   Include "TableWalkSpecMid.v". *)
(*   Include "TableWalkSpec.v". *)
(* End TableWalk. *)
...
(*   Hint InitRely __table_is_uniform_block_loop777 ((v_s2tte_is_x.(pbase) = "s2tte_is_assigned") \/ *)
(*                                               (v_s2tte_is_x.(pbase) = "s2tte_is_destroyed")). *)
...
(*   Include "TableBlockSpecMid.v". *)
(*   Include "TableBlockSpec.v". *)
(* End TableBlock. *)
...
(*       nil. *)
(*   Include "RTTFoldSpecMid.v". *)
(*   Include "RTTFoldSpec.v". *)
(* End RTTFold. *)
...
(*   Hint NoTrans realm_ipa_to_pa_spec_mid. *)
(*   Hint NoUnfold realm_ipa_to_pa_spec_mid. *)
(*   Hint NoUnfold realm_ipa_to_pa_spec. *)
(*   Hint NoTrans realm_ipa_get_ripas_spec_mid. *)
...
(*   Include "S2TTInitSpecMid.v". *)
(*   Include "S2TTInitSpec.v". *)
(* End S2TTInit. *)
...
(*       nil. *)
(*   Include "ValidateFeatureRegSpecMid.v". *)
(*   Include "ValidateFeatureRegSpec.v". *)
...
(*   Hint InitRely free_sl_rtts (v_g_rtt.(pbase) = "granules"). *)
(*   Hint InitRely free_sl_rtts_loop193 (v_g_rtt.(pbase) = "granules"). *)

(*   (* Need fill in: *) *)
(*   Definition total_root_rtt_refcount_loop348_rank (v_g_rtt: Ptr) (v_indvars_iv: Z) (v_refcount_08: Z) (v_wide_trip_count: Z) : Z := *)
(*     v_wide_trip_count - v_indvars_iv. *)
(*   Hint InitRely total_root_rtt_refcount (v_g_rtt.(pbase) = "granules"). *)
(*   Hint InitRely total_root_rtt_refcount_loop348 (v_g_rtt.(pbase) = "granules"). *)
(*   Include "RealmCreateSpecMid.v". *)
(*   Include "RealmCreateSpec.v". *)
(* End RealmCreate. *)
...
(*   Hint InitRely get_realm_params (v_realm_params.(pbase) = "smc_realm_create_stack"). *)
(*   Hint InitRely validate_realm_params (v_p.(pbase) = "smc_realm_create_stack"). *)
(*   Include "RealmParamsSpecMid.v". *)
(*   Include "RealmParamsSpec.v". *)
(* End RealmParams. *)
...
(*       nil. *)
(*   Include "EL3IFCSpecMid.v". *)
(*   Include "EL3IFCSpec.v". *)
(* End EL3IFC. *)
...
(*   (* Hint InitRely complete_psci_cpu_on (v_target_rec.(pbase) = "slot_rec2" /\ v_target_rec.(poffset) = 0). *) *)
(*   (* Hint InitRely complete_psci_affinity_info (v_target_rec.(pbase) = "slot_rec2" /\ v_target_rec.(poffset) = 0). *) *)

(*   Include "PSCIOpsLow.v". *)
(*   Include "PSCIOpsSpecMid.v". *)
(*   Include "PSCIOpsSpec.v". *)
(* End PSCIOps. *)
...
(*       "system_off_reboot" :: *)
(*       nil. *)
(*   (* Hint InitRely psci_complete_request (v_calling_rec.(pbase) = "slot_rec" /\ v_calling_rec.(poffset) = 0). *) *)
(*   (* Hint InitRely psci_complete_request (v_target_rec.(pbase) = "slot_rec2" /\ v_target_rec.(poffset) = 0). *) *)

(*   (* Hint InitRely system_off_reboot (v_rec.(pbase) = "slot_rec" /\ v_rec.(offset) = 0). *) *)

(*   Include "PSCILow.v". *)
(*   Include "PSCISpecMid.v". *)
(*   Include "PSCISpec.v". *)
(* End PSCI. *)
...
(*       "psci_version" :: *)
(*       nil. *)
(*   Hint InitRely psci_features (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_features (v_rec.(pbase) = "slot_rec"). *)

(*   Hint InitRely psci_system_reset (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_system_reset (v_rec.(pbase) = "slot_rec"). *)

(*   Hint InitRely psci_system_off (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_system_off (v_rec.(pbase) = "slot_rec"). *)

(*   Hint InitRely psci_affinity_info (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_affinity_info (v_rec.(pbase) = "slot_rec"). *)

(*   Hint InitRely psci_cpu_on (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_cpu_on (v_rec.(pbase) = "slot_rec"). *)

(*   Hint InitRely psci_cpu_off (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_cpu_off (v_rec.(pbase) = "slot_rec"). *)

(*   Hint InitRely psci_cpu_suspend (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_cpu_suspend (v_rec.(pbase) = "slot_rec"). *)

(*   Hint InitRely psci_version (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely psci_version (v_rec.(pbase) = "slot_rec"). *)
(*   Include "PSCIHandlerLow.v". *)
(*   Include "PSCIHandlerSpecMid.v". *)
(*   Include "PSCIHandlerSpec.v". *)
(* End PSCIHandler. *)
...
(*   Include "RSIOpsLow.v". *)
(*   Include "RSIOpsSpecMid.v". *)
(*   Include "RSIOpsSpec.v". *)
(* End RSIOps. *)
...
(*       nil. *)
(*   Include "RSIHandlerLow.v". *)
(*   Include "RSIHandlerSpecMid.v". *)
(*   Include "RSIHandlerSpec.v". *)
(* End RSIHandler. *)
...
(*   Hint InitRely ipa_is_empty (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
(*   Hint InitRely ipa_is_empty (rec_is_locked st). *)
(*   Hint InitRely get_dabt_write_value (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
(*   Hint InitRely handle_sync_external_abort (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
(*   Hint InitRely handle_sync_external_abort (v_rec_exit.(pbase) = "smc_rec_enter_stack" /\ v_rec_exit.(poffset) = 0). *)
(*   Hint InitRely emulate_sysreg_access_ns (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
(*   Hint InitRely emulate_sysreg_access_ns (v_rec_exit.(pbase) = "smc_rec_enter_stack" /\ v_rec_exit.(poffset) = 0). *)
(*   Hint InitRely handle_rsi_ipa_state_get (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely handle_rsi_ipa_state_get (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
(*   Hint InitRely verify_input_parameters_consistency (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
(*   Hint InitRely psci_rsi (v_agg_result.(pbase) = "handle_realm_rsi_stack"). *)
(*   Hint InitRely data_create (v_g_src.(pbase) = "granules"). *)

(*   (* Hint NoTrans handle_rsi_ipa_state_get_spec_mid. *) *)
(*   (* Hint NoTrans data_create_spec_mid. *) *)

(*   (* Include "data_create_tmp.v". *) *)
(*   Include "ExceptionOpsLow.v". *)
(*   Include "ExceptionOpsSpecMid.v". *)
(*   Include "ExceptionOpsSpec.v". *)
(* End ExceptionOps. *)
...
(*     None. *)
(*   Hint NoTrans handle_sysreg_access_trap_spec_mid. *)
(*   Hint NoTrans handle_rsi_realm_config_spec_mid. *)

(*   Hint NoUnfold handle_sysreg_access_trap_spec. *)
(*   Hint NoUnfold handle_rsi_realm_config_spec. *)

(*   Include "ExceptionHandlerSpecMid.v". *)
(*   Include "ExceptionHandlerSpec.v". *)
...
(*   Hint InitRely handle_realm_rsi (v_rec.(pbase) = "slot_rec" /\ v_rec.(poffset) = 0). *)
(*   Hint InitRely handle_realm_rsi (v_rec_exit.(pbase) = "smc_rec_enter_stack"). *)
(*   Hint NoTrans handle_realm_rsi_spec_mid. *)
(*   Hint NoTrans handle_realm_rsi_loop0_mid. *)
(*   Hint NoTrans handle_realm_rsi_loop0_0_mid. *)
(*   Hint NoTrans handle_realm_rsi_loop0_1_mid. *)
(*   Hint NoTrans handle_realm_rsi_loop0_2_mid. *)
(* End HandleRSI. *)
