Definition granule_lock_spec_mid (v_g: Ptr) (v_expected_state: Z) (st: RData) : (option RData) :=
...
Definition granule_unlock_spec_mid (v_g: Ptr) (st: RData) : (option RData) :=
  (anno (((16 * (0)) = (0)));
  (anno (((0 + (0)) = (0)));
...
Definition find_lock_granule_spec_mid (v_addr: Z) (v_expected_state: Z) (st: RData) : (option (Ptr * RData)) :=
  when ret == ((find_granule_spec' v_addr st));
  (anno ((((ret.(poffset)) - (0)) = ((ret.(poffset)))));
