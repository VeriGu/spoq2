...
Parameter RecInfo_init: RData.

Section RecInfo_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition RecInfo_newframe (fname: string) (st: RData) : option (RData) := None.

  Definition RecInfo_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition RecInfo_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition RecInfo_get_flag (f: flag) (st: RData) : bool := false.

  Definition RecInfo_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition RecInfo_layer :=
    {|
      State := RData;
      Init := RecInfo_init;
      Load := load_RData;
      Store := store_RData;
      NewFrame := RecInfo_newframe;
      Alloca := alloc_stack;
      Free := free_stack;
      GetReg :=RecInfo_get_reg;
      SetReg := RecInfo_set_reg;
      GetFlag := RecInfo_get_flag;
      SetFlag := RecInfo_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
