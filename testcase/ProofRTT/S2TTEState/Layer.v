...
Parameter S2TTEState_init: RData.

Section S2TTEState_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition S2TTEState_newframe (fname: string) (st: RData) : option (RData) := None.

  Definition S2TTEState_alloca (fname: string) (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition S2TTEState_free (fname: string) (init_st: RData) (st: RData) : option RData := None.

  Definition S2TTEState_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition S2TTEState_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition S2TTEState_get_flag (f: flag) (st: RData) : bool := false.

  Definition S2TTEState_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition S2TTEState_layer :=
    {|
      State := RData;
      Init := S2TTEState_init;
      Load := load_RData;
      Store := store_RData;
      NewFrame := S2TTEState_newframe;
      Alloca := S2TTEState_alloca;
      Free := S2TTEState_free;
      GetReg :=S2TTEState_get_reg;
      SetReg := S2TTEState_set_reg;
      GetFlag := S2TTEState_get_flag;
      SetFlag := S2TTEState_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
