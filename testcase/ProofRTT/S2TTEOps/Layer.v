...
Parameter S2TTEOps_init: RData.

Section S2TTEOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition S2TTEOps_newframe (fname: string) (st: RData) : option (RData) := None.

  Definition S2TTEOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition S2TTEOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition S2TTEOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition S2TTEOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition S2TTEOps_layer :=
    {|
      State := RData;
      Init := S2TTEOps_init;
      Load := load_RData;
      Store := store_RData;
      NewFrame := S2TTEOps_newframe;
      Alloca := alloc_stack;
      Free := free_stack;
      GetReg :=S2TTEOps_get_reg;
      SetReg := S2TTEOps_set_reg;
      GetFlag := S2TTEOps_get_flag;
      SetFlag := S2TTEOps_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
      ...
      PrimCall :=
          ...
          :: ("rec_par_size", prim rec_par_size_spec)
          :: ("s2_addr_to_idx", prim s2_addr_to_idx_spec)
          :: ("s2_sl_addr_to_idx", prim s2_sl_addr_to_idx_spec)
          :: ("s2_walk_result_match_ripas", prim s2_walk_result_match_ripas_spec)
