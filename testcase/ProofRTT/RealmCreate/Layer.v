...
Parameter RealmCreate_init: RData.

Section RealmCreate_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition RealmCreate_newframe (fname: string) (st: RData) : option (RData) := None.

  Definition RealmCreate_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition RealmCreate_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition RealmCreate_get_flag (f: flag) (st: RData) : bool := false.

  Definition RealmCreate_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition RealmCreate_layer :=
    {|
      State := RData;
      Init := RealmCreate_init;
      Load := load_RData;
      Store := store_RData;
      NewFrame := RealmCreate_newframe;
      Alloca := alloc_stack;
      Free := free_stack;
      GetReg :=RealmCreate_get_reg;
      SetReg := RealmCreate_set_reg;
      GetFlag := RealmCreate_get_flag;
      SetFlag := RealmCreate_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
