...
Parameter RTTFold_init: RData.

Section RTTFold_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition RTTFold_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition RTTFold_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition RTTFold_get_flag (f: flag) (st: RData) : bool := false.

  Definition RTTFold_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition RTTFold_layer :=
    {|
      State := RData;
      Init := RTTFold_init;
      Load := load_RData;
      ...
      Alloca := alloc_stack;
      Free := free_stack;
      GetReg :=RTTFold_get_reg;
      SetReg := RTTFold_set_reg;
      GetFlag := RTTFold_get_flag;
      SetFlag := RTTFold_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
