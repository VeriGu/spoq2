...
Lemma smc_rtt_unmap_unprotected_secure:
      forall sec_d sec_st norm_d (Hrel: relate sec_d sec_st norm_d)
        v_rd_addr v_map_addr v_ulevel ret_s ret_n sec_d' norm_d'
        (Hsec: smc_rtt_unmap_unprotected_spec v_rd_addr v_map_addr v_ulevel sec_d = Some (ret_s, sec_d'))
        (Hnorm: smc_rtt_unmap_unprotected_spec v_rd_addr v_map_addr v_ulevel norm_d = Some (ret_n, norm_d'))
...
Proof.
  ...
  unfold smc_rtt_unmap_unprotected_spec in *; autounfold with sem in *.
  all: repeat rewrite oracle_rel in *. repeat rewrite_id.
  do 5 simpl_hyp Hsec.
  ...
  - repeat rewrite_id. repeat rewrite lens_same in *.
    ...
    + simpl_hyp Hsec. simpl_hyp Hsec. simpl_hyp Hsec.
      ...
      extract_match Hsec Hs2tte_check.
