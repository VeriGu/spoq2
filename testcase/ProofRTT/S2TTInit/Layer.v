...
Parameter S2TTInit_init: RData.

Section S2TTInit_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition S2TTInit_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition S2TTInit_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition S2TTInit_get_flag (f: flag) (st: RData) : bool := false.

  Definition S2TTInit_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition S2TTInit_layer :=
    {|
      State := RData;
      Init := S2TTInit_init;
      Load := load_RData;
      ...
      Alloca := alloc_stack;
      Free := free_stack;
      GetReg :=S2TTInit_get_reg;
      SetReg := S2TTInit_set_reg;
      GetFlag := S2TTInit_get_flag;
      SetFlag := S2TTInit_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
      ...
      PrimCall :=
          ...
          :: ("realm_ipa_get_ripas", prim realm_ipa_get_ripas_spec)
          :: ("realm_ipa_to_pa", prim realm_ipa_to_pa_spec)
