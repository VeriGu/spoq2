...
Section S2TTInit_s2tt_init_valid_LowSpec.

  Context `{int_ptr: IntPtrCast}.

  Fixpoint s2tt_init_valid_loop719_low (_N_: nat) (__return__: bool) (v_call: Z) (v_indvars_iv: Z) (v_level: Z) (v_pa_addr_05: Z) (v_s2tt: Ptr) (st: RData) : (option (bool * Z * Z * Z * Z * Ptr * RData)) :=
    match (_N_) with
    | O => (Some (__return__, v_call, v_indvars_iv, v_level, v_pa_addr_05, v_s2tt, st))
    | (S _N_) =>
      match ((s2tt_init_valid_loop719_low _N_ __return__ v_call v_indvars_iv v_level v_pa_addr_05 v_s2tt st)) with
      | (Some (__return__, v_call, v_indvars_iv, v_level, v_pa_addr_05, v_s2tt, st)) =>
        ...
        then (Some (__return__, v_call, v_indvars_iv, v_level, v_pa_addr_05, v_s2tt, st))
          ...
          when v_call2, st == ((s2tte_create_valid_spec v_pa_addr_05 v_level st));
  ...
  Definition s2tt_init_valid_spec_low (v_s2tt: Ptr) (v_pa: Z) (v_level: Z) (st: RData) : (option RData) :=
    rely (((v_s2tt.(pbase)) = ("slot_delegated")));
    let v_conv := v_level in
    when v_call, st == ((s2tte_map_size_spec v_conv st));
    let v_indvars_iv := 0 in
    let v_pa_addr_05 := v_pa in
    rely (((s2tt_init_valid_loop719_rank v_call v_indvars_iv v_level v_pa_addr_05 v_s2tt) >= (0)));
    match (
      (s2tt_init_valid_loop719_low
        (z_to_nat (s2tt_init_valid_loop719_rank v_call v_indvars_iv v_level v_pa_addr_05 v_s2tt))
        false
        v_call
        v_indvars_iv
        v_level
...
End S2TTInit_s2tt_init_valid_LowSpec.

#[global] Hint Unfold s2tt_init_valid_loop719_low: spec.
