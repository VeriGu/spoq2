...
Section MmapInternal_Spec.
  ...
  Definition buffer_map_internal_spec (v_slot: Z) (v_addr: Z) (st: RData) : (option (Ptr * RData)) :=
    ...
    rely ((((0 - (CPU_ID)) <= (0)) /\ ((CPU_ID < (16)))));
    if (v_slot =? (0))
    then (Some ((mkPtr "slot_ns" 0), (st.[share].[slots] :< (((st.(share)).(slots)) # SLOT_NS == (v_addr / (GRANULE_SIZE))))))
    else (
      if (
        if ((((((v_slot << (12)) + (18446744073709420544)) - (SLOT_VIRT)) / (GRANULE_SIZE)) - (SLOT_DELEGATED)) =? (0))
        then true
        else false)
      then (Some ((mkPtr "slot_delegated" 0), (st.[share].[slots] :< (((st.(share)).(slots)) # SLOT_DELEGATED == (v_addr / (GRANULE_SIZE))))))
      else (
        if (
...
End MmapInternal_Spec.

#[global] Hint Unfold granule_addr_spec: spec.
#[global] Hint Unfold buffer_unmap_internal_spec: spec.
#[global] Hint Unfold buffer_map_internal_spec: spec.
