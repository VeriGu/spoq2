...
Section MemRW_ns_buffer_write_LowSpec.

  Context `{int_ptr: IntPtrCast}.

  Definition ns_buffer_write_spec_low (v_slot: Z) (v_ns_gr: Ptr) (v_offset: Z) (v_size: Z) (v_src: Ptr) (st: RData) : (option (bool * RData)) :=
    rely ((v_offset = (0)));
    ...
    when v_call5, st == ((memcpy_ns_write_spec v_2 v_src v_conv4 st));
    when st == ((ns_buffer_unmap_spec v_call st));
    let __return__ := true in
    let __retval__ := v_call5 in
    (Some (__retval__, st)).

End MemRW_ns_buffer_write_LowSpec.
