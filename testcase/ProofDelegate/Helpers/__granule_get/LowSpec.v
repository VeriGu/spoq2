...
Section Helpers___granule_get_LowSpec.

  Context `{int_ptr: IntPtrCast}.

  Definition __granule_get_spec_low (v_g: Ptr) (st: RData) : (option RData) :=
    ...
    let v_inc := (v_0 + (1)) in
    when st == ((store_RData 8 v_refcount v_inc st));
    let __return__ := true in
    (Some st).

End Helpers___granule_get_LowSpec.
