Definition update_ripas_spec_mid (v_s2tte: Ptr) (v_level: Z) (v_ripas: Z) (st: RData) : (option (bool * RData)) :=
...
Definition s2tte_create_unassigned_spec_mid (v_ripas: Z) (st: RData) : (option (Z * RData)) :=
  (Some ((s2tte_create_ripas_spec' v_ripas st), st)).

Definition realm_ipa_size_spec_mid (v_rd: Ptr) (st: RData) : (option (Z * RData)) :=
...
Definition __find_next_level_idx_spec_mid (v_g_tbl: Ptr) (v_idx: Z) (st: RData) : (option (Ptr * RData)) :=
  rely (
...
Definition s2_walk_result_match_ripas_spec_mid (v_res: Ptr) (v_ripas: Z) (st: RData) : (option (bool * RData)) :=
  rely (
  ...
  then (
    ...
    then (
      ...
      (anno (((16 + (0)) = (16)));
      (anno (((0 + (16)) = (16)));
      (Some (((((st.(stack)).(attest_token_continue_write_state_stack)) @ ((v_res.(poffset)) + (16))) =? (0)), st))))))
    else (Some (false, st)))
  else (
    ...
    then (
      ...
      then (
        ...
        (anno (((16 + (0)) = (16)));
        (anno (((0 + (16)) = (16)));
        (Some (((((st.(stack)).(do_host_call_stack)) @ ((v_res.(poffset)) + (16))) =? (0)), st))))))
      else (Some (false, st)))
    else (
...
Definition rec_par_size_spec_mid (v_rec: Ptr) (st: RData) : (option (Z * RData)) :=
  rely (
