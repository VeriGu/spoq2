Fixpoint rtt_walk_lock_unlock_loop370_mid (_N_: nat) (__return__: bool) (v_g_tbls: Ptr) (v_indvars_iv: Z) (v_level: Z) (v_map_addr: Z) (v_wi: Ptr) (st: RData) : (option (bool * Ptr * Z * Z * Z * Ptr * RData)) :=
  match (_N_) with
  | O => (Some (__return__, v_g_tbls, v_indvars_iv, v_level, v_map_addr, v_wi, st))
  | (S _N_) =>
    match ((rtt_walk_lock_unlock_loop370_mid _N_ __return__ v_g_tbls v_indvars_iv v_level v_map_addr v_wi st)) with
    | (Some (__return__, v_g_tbls, v_indvars_iv, v_level, v_map_addr, v_wi, st)) =>
      ...
      if __return__
      then (Some (__return__, v_g_tbls, v_indvars_iv, v_level, v_map_addr, v_wi, st))
      else (
        ...
        | (Some (__break__, __continue__, v_indvars_iv, st)) =>
          ...
          then (Some (__return__, v_g_tbls, v_indvars_iv, v_level, v_map_addr, v_wi, st))
            ...
            if __continue__
            then (Some (__return__, v_g_tbls, v_indvars_iv, v_level, v_map_addr, v_wi, st))
            else (Some (__return__, v_g_tbls, v_indvars_iv, v_level, v_map_addr, v_wi, st)))
        | None => None
        end)
    | None => None
    end
...
Definition rtt_walk_lock_unlock_spec_mid (v_g_root: Ptr) (v_start_level: Z) (v_ipa_bits: Z) (v_map_addr: Z) (v_level: Z) (v_wi: Ptr) (st: RData) : (option RData) :=
  ...
  when __return__, st == (
      ...
      then (
        ...
        match (
          (rtt_walk_lock_unlock_loop370_mid
