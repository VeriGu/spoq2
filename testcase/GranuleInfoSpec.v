...
Fixpoint find_lock_granules_loop197 (_N_: nat) (__break__: bool) (v_cmp235: bool) (v_granules: Ptr) (v_i_241: Z) (st: RData) : (option (bool * bool * Ptr * Z * RData)) :=
  match (_N_) with
  | O => (Some (__break__, v_cmp235, v_granules, v_i_241, st))
  | (S _N_) =>
    match ((find_lock_granules_loop197 _N_ __break__ v_cmp235 v_granules v_i_241 st)) with
...
Fixpoint find_lock_granules_loop0 (_N_: nat) (__return__: bool) (__retval__: bool) (v_granules: Ptr) (v_i_143: Z) (st: RData) : (option (bool * bool * Ptr * Z * RData)) :=
  match (_N_) with
  | O => (Some (__return__, __retval__, v_granules, v_i_143, st))
  | (S _N_) =>
    match ((find_lock_granules_loop0 _N_ __return__ __retval__ v_granules v_i_143 st)) with
          ...
          then (
            ...
            match (
              ...
              then (
                ...
                match ((find_lock_granules_loop197 (z_to_nat (find_lock_granules_loop197_rank v_granules v_i_241)) false false v_granules v_i_241 st)) with
                | (Some (__break__, v_cmp235, v_granules, v_i_241, st)) =>
                  if __break__
                  then (
...
Definition find_lock_granules_spec (v_granules: Ptr) (v_n: Z) (st: RData) : (option (bool * RData)) :=
  ...
  match ((find_lock_granules_loop0 (z_to_nat (find_lock_granules_loop0_rank v_granules v_i_143)) false false v_granules v_i_143 st)) with
  | (Some (__return__, __retval__, v_granules, v_i_143, st)) =>
    if __return__
