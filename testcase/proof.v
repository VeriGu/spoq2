Definition PROJ_NAME: string := "RMMProof.ProofGenHigh".
Definition PROJ_BASE: string := "./ProofGenHigh".
...
Definition StackDataTypeGranuleSet : Z := 0.
Definition StackDataTypeGranuleSetTwo : Z := 1.
Definition StackDataTypeS2TTE : Z := 2.
Definition StackDataTypeS2WalkResult : Z := 3.
Definition StackDataTypeRecParams : Z := 4.
...
Record PerCPUStackData :=
  mkPerCPUStackData {
      stack_data_type: Z;
      stack_data_granule_set: s_granule_set;
      (* tuple fields in a record are not well-supported by Spoq... *)
      (* stack_data_granule_set_two: (s_granule_set * s_granule_set) *)
      stack_data_granule_set_arr_0: s_granule_set;
      stack_data_granule_set_arr_1: s_granule_set;
      stack_data_s2tte: Z;
      stack_data_s2_walk_result: s_s2_walk_result;
      stack_data_rec_params: s_rmi_rec_params
    }.

Record PerCPU :=
  mkPerCPU {
      (* Each alloca allocate a slot in the stack of a specific type *)
      pcpu_used_stack_slots: Z; (* First available slot *)
      pcpu_stack_data_slots: ZMap.t PerCPUStackData;
      (* EL2 Regs *)
      pcpu_vttbr_el2: Z;
      pcpu_zcr_el2: Z;
      pcpu_cnthctl_el2: Z;
...
Definition rec_field_accessible (rec: s_rec) (ofs: Z) (rec_granule: s_granule) (locked: bool) (st: RData): bool :=
  let rec_g_rd := int_to_ptr rec.(e_realm_info).(e_g_rd) in
  match (rec_g_rd.(pbase), rec_g_rd.(poffset)) with
  | ("granules", ofs) =>
      ...
      if (ofs >=? 848) && (ofs <? 880) then
        ...
        if g.(e_refcount) =? 0 then
          ...
          false
  | _ => locked (* if no RD associate with the REC, it is initialization *)
  end.

(* XXX: think about this *)
Definition rd_field_accessible (rd: s_rd) (ofs: Z) (rd_granule: s_granule) (st: RData) : bool :=
...
Definition load_RData (sz: Z) (p: Ptr) (st: RData) : (option (Z * RData)) :=
  ...
  | ("gic_virt_feature_0", _) => Some (gic_virt_feature_0, st)
  | ("gic_virt_feature_1", _) => Some (gic_virt_feature_1, st)
  | ("gic_virt_feature_2", _) => Some (gic_virt_feature_2, st)
  | ("gic_virt_feature_3", _) => Some (gic_virt_feature_3, st)
  | ("gic_virt_feature_4", _) => Some (gic_virt_feature_4, st)
  | ("granules", ofs) =>
      let idx := ofs / ST_GRANULE_SIZE in
  ...
  | ("slot_rd", ofs) =>
      ...
      rely (rd_field_accessible g_data.(g_rd) ofs g st = true);
      match g.(e_lock) with
      | Some cid =>
  ...
  | ("slot_rec", ofs) =>
      ...
      rely (rec_field_accessible g_data.(g_rec) ofs g locked st = true);
      let st :=
  ...
  | ("slot_rec_aux0", ofs) =>
       ...
       *)
      rely (rec_field_accessible parent_g_data.(g_rec) 1096 parent_g false st = true);
      (* Attestation HEAP *)
  ...
  | ("stack", ofs) =>
      ...
      let slot_data := (st.(priv).(pcpu_stack_data_slots) @ slot_nr) in
      if slot_data.(stack_data_type) =? StackDataTypeGranuleSet then (
        when ret == load_s_granule_set sz slot_ofs slot_data.(stack_data_granule_set);
        Some (ret, st) ) else
      if slot_data.(stack_data_type) =? StackDataTypeGranuleSetTwo then (
        let arr_idx := slot_ofs / s_granule_set_size in
        rely (arr_idx < 2);
        if arr_idx =? 0 then
          when ret == load_s_granule_set sz slot_ofs slot_data.(stack_data_granule_set_arr_0);
          Some (ret, st)
        else
          when ret == load_s_granule_set sz (slot_ofs - s_granule_set_size) slot_data.(stack_data_granule_set_arr_1);
          Some (ret, st) ) else
      if slot_data.(stack_data_type) =? StackDataTypeS2TTE then (
        Some (slot_data.(stack_data_s2tte), st)
        ) else
      if slot_data.(stack_data_type) =? StackDataTypeS2WalkResult then (
        when ret == load_s_s2_walk_result sz slot_ofs slot_data.(stack_data_s2_walk_result);
        Some (ret, st) ) else
      if slot_data.(stack_data_type) =? StackDataTypeRecParams then (
        when ret == load_s_rmi_rec_params sz slot_ofs slot_data.(stack_data_rec_params);
        Some (ret, st) )
        (* match slot_data.(stack_data_granule_set_two) with *)
        (* | (fst, snd) => *)
        (*     if arr_idx =? 0 then *)
        (*       when ret == load_s_granule_set sz slot_ofs fst; *)
        (*       Some (ret, st) *)
        (*     else *)
        (*       when ret == load_s_granule_set sz (slot_ofs - s_granule_set_size) snd; *)
        (*       Some (ret, st) *)
        (* end *)
      else None
  (* Global variables *)
  | ("__const_arch_feat_get_pa_width_pa_range_bits_arr", ofs) =>
      ...
      if ofs =? 24 then Some (52, st) else
        None
  | ("g_sve_max_vq", ofs) => Some (st.(share).(gv_g_sve_max_vq), st)
  | ("g_ns_simd", ofs) =>
  ...
  | ("sl0_val", ofs) =>
      ...
      if ofs =? 24 then Some (192, st) else None
  | _ => None
  end.
...
Definition store_RData (sz: Z) (p: Ptr) (v: Z) (st: RData) : option RData :=
  ...
  | ("slot_rec_aux0", ofs) =>
       ...
       * rec->aux_data instead.
       *)
      rely (rec_field_accessible parent_g_data.(g_rec) 1096 parent_g true st = true);
      (* Attestation HEAP *)
  ...
  | ("stack", ofs) =>
      ...
      let slot_data := (st.(priv).(pcpu_stack_data_slots) @ slot_nr) in
      if slot_data.(stack_data_type) =? StackDataTypeGranuleSet then (
        when ret == store_s_granule_set sz slot_ofs v slot_data.(stack_data_granule_set);
        let new_slot_data := slot_data.[stack_data_granule_set] :< ret in
        Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # slot_nr == new_slot_data)) ) else
      if slot_data.(stack_data_type) =? StackDataTypeGranuleSetTwo then (
        let arr_idx := slot_ofs / s_granule_set_size in
        rely (arr_idx < 2);
        if arr_idx =? 0 then
          when new_fst == store_s_granule_set sz slot_ofs v slot_data.(stack_data_granule_set_arr_0);
          let new_slot_data := slot_data.[stack_data_granule_set_arr_0] :< new_fst in
          Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # slot_nr == new_slot_data))
        else
          when new_snd == store_s_granule_set sz (slot_ofs - s_granule_set_size) v slot_data.(stack_data_granule_set_arr_1);
          let new_slot_data := slot_data.[stack_data_granule_set_arr_1] :< new_snd in
          Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # slot_nr == new_slot_data)) ) else
      if slot_data.(stack_data_type) =? StackDataTypeS2TTE then (
        let new_slot_data := slot_data.[stack_data_s2tte] :< v in
        Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # slot_nr == new_slot_data)) ) else
      if slot_data.(stack_data_type) =? StackDataTypeS2WalkResult then (
        when ret == store_s_s2_walk_result sz slot_ofs v slot_data.(stack_data_s2_walk_result);
        let new_slot_data := slot_data.[stack_data_s2_walk_result] :< ret in
        Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # slot_nr == new_slot_data)) )
        (* match slot_data.(stack_data_granule_set_two) with *)
        (* | (fst, snd) => *)
        (*     if arr_idx =? 0 then *)
        (*       when new_fst == store_s_granule_set sz slot_ofs v fst; *)
        (*       let new_slot_data := slot_data.[stack_data_granule_set_two] :< (new_fst, snd) in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # slot_nr == new_slot_data)) *)
        (*     else *)
        (*       when new_snd == store_s_granule_set sz (slot_ofs - s_granule_set_size) v snd; *)
        (*       let new_slot_data := slot_data.[stack_data_granule_set_two] :< (fst, new_snd) in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # slot_nr == new_slot_data)) *)
        (* end *)
      else None
  (* Global variables *)
  | ("g_sve_max_vq", ofs) => Some (st.[share].[gv_g_sve_max_vq] :< v)
  | ("g_ns_simd", ofs) =>
...
Definition base_to_slot (b: string) : option Z :=
  match b with
  | "slot_ns" => Some SLOT_NS
  | "slot_delegated" => Some SLOT_DELEGATED
  | "slot_rd" => Some SLOT_RD
  | "slot_rec" => Some SLOT_REC
  | "slot_rec2" => Some SLOT_REC2
  | "slot_rec_target" => Some SLOT_REC_TARGET
  | "slot_rec_aux0" => Some SLOT_REC_AUX0
  | "slot_rtt" => Some SLOT_RTT
  | "slot_rtt2" => Some SLOT_RTT2
  | "slot_rsi_call" => Some SLOT_RSI_CALL
  | _ => None
  end.
...
Definition slot_to_ptr (slot: Z) (ofs: Z) : Ptr :=
  ...
  if (slot >=? SLOT_REC_AUX0) && (slot <? SLOT_REC_AUX0 + MAX_REC_AUX_GRANULES) then (mkPtr "slot_rec_aux0" ofs) else
...
Section Bottom.
  ...
  Definition spinlock_acquire_spec (lock: Ptr) (st: RData) : option RData :=
    match (lock.(pbase), lock.(poffset)) with
    | ("granules", ofs) =>
  ...
  Definition llvm_memcpy_p0i8_p0i8_i64_spec (v_dest: Ptr) (v_src: Ptr) (sz: Z) (is_volatile: bool) (st: RData) : option RData :=
    if (v_dest.(pbase) =s "stack") && (v_src.(pbase) =s "stack") then
      ...
      let dest_slot_data := (st.(priv).(pcpu_stack_data_slots) @ dest_slot_nr) in
      let dest_slot_type := dest_slot_data.(stack_data_type) in
      let src_slot_nr := stack_ptr_extract_slot v_src.(poffset) in
      let src_slot_ofs := stack_ptr_extract_ofs v_src.(poffset) in
      let src_slot_data := (st.(priv).(pcpu_stack_data_slots) @ src_slot_nr) in
      let src_slot_type := src_slot_data.(stack_data_type) in
      if (dest_slot_type =? StackDataTypeGranuleSet) && (src_slot_type =? StackDataTypeGranuleSetTwo) then (
        (* sort_granules: tmp = granules[i] *)
        rely (sz = 24);
        let arr_idx := src_slot_ofs / s_granule_set_size in
        rely (arr_idx < 2);
        if arr_idx =? 0 then
              let new_slot_data := dest_slot_data.[stack_data_granule_set] :< src_slot_data.(stack_data_granule_set_arr_0) in
              Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data))
        else
          let new_slot_data := dest_slot_data.[stack_data_granule_set] :< src_slot_data.(stack_data_granule_set_arr_1) in
          Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data))
        ) else
        (* match src_slot_data.(stack_data_granule_set_two) with *)
        (* | (fst, snd) => *)
        (*     if arr_idx =? 0 then *)
        (*       let new_slot_data := dest_slot_data.[stack_data_granule_set] :< fst in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data)) *)
        (*     else *)
        (*       let new_slot_data := dest_slot_data.[stack_data_granule_set] :< snd in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data)) *)
        (* end else *)
      if (dest_slot_type =? StackDataTypeGranuleSetTwo) && (src_slot_type =? StackDataTypeGranuleSetTwo) then (
        (* sort_granules: granules[j] = granules[j - 1] *)
        rely (sz = 40);
        let src_arr_idx := src_slot_ofs / s_granule_set_size in
        let dest_arr_idx := dest_slot_ofs / s_granule_set_size in
        rely (src_arr_idx < 2 /\ dest_arr_idx < 2);
        if src_arr_idx =? 0 then
          let new_slot_data := dest_slot_data.[stack_data_granule_set_arr_1] :< src_slot_data.(stack_data_granule_set_arr_0) in
          Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data))
        else
          let new_slot_data := dest_slot_data.[stack_data_granule_set_arr_0] :< src_slot_data.(stack_data_granule_set_arr_1) in
          Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data))
        ) else
        (* match src_slot_data.(stack_data_granule_set_two) with *)
        (* | (fst, snd) => *)
        (*     if src_arr_idx =? 0 then *)
        (*       let new_slot_data := dest_slot_data.[stack_data_granule_set_two] :< (fst, fst) in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data)) *)
        (*     else *)
        (*       let new_slot_data := dest_slot_data.[stack_data_granule_set_two] :< (snd, snd) in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data)) *)
        (* end else *)
      if (dest_slot_type =? StackDataTypeGranuleSetTwo) && (src_slot_type =? StackDataTypeGranuleSet) then (
        (* sort_granules: granules[j] = temp *)
        rely (sz = 24);
        let arr_idx := dest_slot_ofs / s_granule_set_size in
        rely (arr_idx < 2);
        if arr_idx =? 0 then
          let new_slot_data := dest_slot_data.[stack_data_granule_set_arr_0] :< src_slot_data.(stack_data_granule_set) in
          Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data))
        else
          let new_slot_data := dest_slot_data.[stack_data_granule_set_arr_1] :< src_slot_data.(stack_data_granule_set) in
          Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data))
        ) else Some st
        (* match dest_slot_data.(stack_data_granule_set_two) with *)
        (* | (fst, snd) => *)
        (*     if arr_idx =? 0 then *)
        (*       let new_slot_data := dest_slot_data.[stack_data_granule_set_two] :< (src_slot_data.(stack_data_granule_set), snd) in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data)) *)
        (*     else *)
        (*       let new_slot_data := dest_slot_data.[stack_data_granule_set_two] :< (fst, src_slot_data.(stack_data_granule_set)) in *)
        (*       Some (st.[priv].[pcpu_stack_data_slots] :< (st.(priv).(pcpu_stack_data_slots) # dest_slot_nr == new_slot_data)) *)
        (* end *)
    else Some st.
  ...
  Definition memcpy_ns_read_spec (v_dest: Ptr) (v_src: Ptr) (v_conv: Z) (st: RData) : option (bool * RData) :=
    match (v_dest.(pbase), v_dest.(poffset)) with
    | ("stack", ofs) =>
        match (v_src.(pbase), v_src.(poffset)) with
        | ("slot_ns", ns_ofs) => Some (true, st)
        | _ => None
        end
    | _ => None
    end.
...
Definition FindGranule_alloc_stack (sz: Z) (align: Z) (st: RData) : option (Ptr * RData) :=
  let free_slot_nr := st.(priv).(pcpu_used_stack_slots) in
  let slot := st.(priv).(pcpu_stack_data_slots) @ free_slot_nr in
  let new_slots := (st.(priv).(pcpu_stack_data_slots)) # free_slot_nr == (slot.[stack_data_type] :< StackDataTypeGranuleSet) in
  Some ((mkPtr "stack" (free_slot_nr << MaxStackOrder)), (st.[priv].[pcpu_used_stack_slots] :< (free_slot_nr + 1)).[priv].[pcpu_stack_data_slots] :< new_slots).
...
Section FindGranule.
  ...
  Definition LAYER_LOAD : string := "load_RData".
  Definition LAYER_STORE : string := "store_RData".
  Definition LAYER_ALLOC : string := "FindGranule_alloc_stack".
  Definition LAYER_FREE : string := "free_stack".
  Definition LAYER_PTR2INT : string := "ptr_to_int".
  Definition LAYER_INT2PTR : string := "int_to_ptr".
  Definition LAYER_PTR_EQB : string := "ptr_eqb".
  ...
  Hint NoTrans granule_lock_on_state_match_spec_mid.
  Hint NoTrans find_granule_spec_mid.
  Hint NoTrans addr_to_granule.

  (* Hint NoTrans sort_granules_spec_mid. *)
  Hint NoUnfold stack_ptr_extract_ofs.
  Hint NoUnfold stack_ptr_extract_slot.
  Hint InitRely granule_lock_on_state_match (v_g.(pbase) = "granules" /\ (v_g.(poffset) mod ST_GRANULE_SIZE) = 0).
  (* Hint NoUnfold load_RData. *)
  (* Hint NoUnfold load_s_granule_set. *)
  (* Hint NoUnfold store_RData. *)
  (* Hint NoUnfold store_s_granule_set. *)
  Hint InitRely sort_granules (StackDataTypeGranuleSet <> StackDataTypeGranuleSetTwo).
  Hint InitRely sort_granules ((stack_ptr_extract_slot (((st.(priv)).(pcpu_used_stack_slots)) << MaxStackOrder)) = ((st.(priv)).(pcpu_used_stack_slots))).
  Hint InitRely sort_granules (v_granules.(pbase) = "stack").
  Hint InitRely sort_granules (stack_ptr_extract_ofs (v_granules.(poffset)) = 0).
  Hint InitRely sort_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 8) = 8).
  Hint InitRely sort_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 16) = 16).
  Hint InitRely sort_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 40) = 40).
  Hint InitRely sort_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 48) = 48).
  Hint InitRely sort_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 56) = 56).
  Hint InitRely sort_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely sort_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 8)).
  Hint InitRely sort_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 16)).
  Hint InitRely sort_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 40)).
  Hint InitRely sort_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 48)).
  Hint InitRely sort_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 56)).
  (*Hint InitRely sort_granules ( v_granules.(poffset) = ((st.(priv).(pcpu_used_stack_slots)) << MaxStackOrder)). *)
  Hint InitRely sort_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset)))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely sort_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 8))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely sort_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 16))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely sort_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 40))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely sort_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 48))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely sort_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 56))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely sort_granules (stack_ptr_extract_slot (v_granules.(poffset) + 8) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely sort_granules (stack_ptr_extract_slot (v_granules.(poffset) + 16) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely sort_granules (stack_ptr_extract_slot (v_granules.(poffset) + 40) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely sort_granules (stack_ptr_extract_slot (v_granules.(poffset) + 48) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely sort_granules (stack_ptr_extract_slot (v_granules.(poffset) + 56) = stack_ptr_extract_slot (v_granules.(poffset))).
...
Section GranuleInfo.
 ...
 (* Need fill in: *)
  ...
  Definition find_lock_granules_loop0_rank (v_granules: Ptr) (v_i_143: Z) : Z :=
    2 - v_i_143.
  Hint InitRely find_lock_granules (v_granules.(pbase) = "stack").
  Hint InitRely find_lock_granules (StackDataTypeGranuleSet <> StackDataTypeGranuleSetTwo).
  Hint InitRely find_lock_granules ((stack_ptr_extract_slot (((st.(priv)).(pcpu_used_stack_slots)) << MaxStackOrder)) = ((st.(priv)).(pcpu_used_stack_slots))).
  Hint InitRely find_lock_granules (stack_ptr_extract_ofs (v_granules.(poffset)) = 0).
  Hint InitRely find_lock_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 8) = 8).
  Hint InitRely find_lock_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 16) = 16).
  Hint InitRely find_lock_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 40) = 40).
  Hint InitRely find_lock_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 48) = 48).
  Hint InitRely find_lock_granules (stack_ptr_extract_ofs (v_granules.(poffset) + 56) = 56).
  Hint InitRely find_lock_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely find_lock_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 8)).
  Hint InitRely find_lock_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 16)).
  Hint InitRely find_lock_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 40)).
  Hint InitRely find_lock_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 48)).
  Hint InitRely find_lock_granules (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_granules.(poffset) + 56)).
  Hint InitRely find_lock_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset)))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely find_lock_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 8))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely find_lock_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 16))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely find_lock_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 40))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely find_lock_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 48))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely find_lock_granules ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_granules.(poffset) + 56))).(stack_data_type) = StackDataTypeGranuleSetTwo).
  Hint InitRely find_lock_granules (stack_ptr_extract_slot (v_granules.(poffset) + 8) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely find_lock_granules (stack_ptr_extract_slot (v_granules.(poffset) + 16) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely find_lock_granules (stack_ptr_extract_slot (v_granules.(poffset) + 40) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely find_lock_granules (stack_ptr_extract_slot (v_granules.(poffset) + 48) = stack_ptr_extract_slot (v_granules.(poffset))).
  Hint InitRely find_lock_granules (stack_ptr_extract_slot (v_granules.(poffset) + 56) = stack_ptr_extract_slot (v_granules.(poffset))).
  (* Hint NoUnfold load_RData. *)
  (* Hint NoUnfold store_RData. *)
  Hint NoUnfold find_lock_granules_loop0.
  Hint NoUnfold find_lock_granules_loop197.
  Hint NoUnfold sort_granules.
  Hint NoUnfold granule_unlock.
  (* Hint InitRely find_lock_rd_granules (v_g_rd.(pbase) = "slot_rd"). *)
...
(*       nil. *)
(*  Need fill in: *)
(*   Definition stage2_tlbi_ipa_loop0_rank (v_call: Z) (v_ipa_addr_05: Z) (v_size_addr_04: Z) : Z := *)
(*     v_size_addr_04. *)
(* End TLBI. *)
...
Section S2TTEOps.
  ...
  (* stack variable *)
  Hint InitRely update_ripas (v_s2tte.(pbase) = "stack").
  (* Hint InitRely update_ripas ((stack_ptr_extract_slot (((st.(priv)).(pcpu_used_stack_slots)) << MaxStackOrder)) = ((st.(priv)).(pcpu_used_stack_slots))). *)
  (* variable type: S2TTE *)
  Hint InitRely update_ripas ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_s2tte.(poffset)))).(stack_data_type) = StackDataTypeS2TTE).
  (* offset always 0 *)
  Hint InitRely update_ripas (stack_ptr_extract_ofs v_s2tte.(poffset) = 0).

  Hint InitRely s2_walk_result_match_ripas (v_res.(pbase) = "stack").
  (* Hint InitRely s2_walk_result_match_ripas ((stack_ptr_extract_slot (((st.(priv)).(pcpu_used_stack_slots)) << MaxStackOrder)) = ((st.(priv)).(pcpu_used_stack_slots))). *)
  Hint InitRely s2_walk_result_match_ripas ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_res.(poffset)))).(stack_data_type) = StackDataTypeS2WalkResult).
  (* Hint InitRely s2_walk_result_match_ripas (st.(priv).(pcpu_used_stack_slots) <> stack_ptr_extract_slot (v_res.(poffset))). *)
  Hint InitRely s2_walk_result_match_ripas (stack_ptr_extract_ofs (v_res.(poffset)) = 0).
  Hint InitRely s2_walk_result_match_ripas (stack_ptr_extract_ofs (v_res.(poffset) + 8) = 8).
  Hint InitRely s2_walk_result_match_ripas (stack_ptr_extract_ofs (v_res.(poffset) + 16) = 16).
  Hint InitRely s2_walk_result_match_ripas (stack_ptr_extract_ofs (v_res.(poffset) + 20) = 20).
  Hint InitRely s2_walk_result_match_ripas (stack_ptr_extract_ofs (v_res.(poffset) + 24) = 24).
  Hint InitRely s2_walk_result_match_ripas ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_res.(poffset) + 8))).(stack_data_type) = StackDataTypeS2WalkResult).
  Hint InitRely s2_walk_result_match_ripas ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_res.(poffset) + 16))).(stack_data_type) = StackDataTypeS2WalkResult).
  Hint InitRely s2_walk_result_match_ripas ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_res.(poffset) + 20))).(stack_data_type) = StackDataTypeS2WalkResult).
  Hint InitRely s2_walk_result_match_ripas ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_res.(poffset) + 24))).(stack_data_type) = StackDataTypeS2WalkResult).
  Hint InitRely s2_walk_result_match_ripas (stack_ptr_extract_ofs v_res.(poffset) = 0).
...
Section InitRegs.
  ...
  (* Hint InitRely init_common_sysregs ((int_to_ptr (st.(share).(granule_data) @ (st.(share).(slots) @ SLOT_REC)).(g_rec).(e_realm_info).(e_g_rd)) = (mkPtr "NULL" 0)). *)
   (* Hint InitRely init_common_sysregs ( *)
   (*    let g_idx := st.(share).(slots) @ SLOT_REC in *)
   (*    let g_data := st.(share).(granule_data) @ g_idx in *)
   (*    let g := st.(share).(granules) @ g_idx in *)
   (*    rec_field_accessible g_data.(g_rec) 816 g true st = true). *)
  (* Hint InitRely init_common_sysregs ( *)
  (*     let g_idx := st.(share).(slots) @ SLOT_REC in *)
  (*     let g_data := st.(share).(granule_data) @ g_idx in *)
  (*     let g := st.(share).(granules) @ g_idx in *)
...
(*     0. *)
(*   Hint NoUnfold init_common_sysregs_spec. *)
...
(*   Hint InitRely init_rec_regs (v_rec_params.(pbase) = "stack"). *)
(*   Hint InitRely init_rec_regs (stack_ptr_extract_ofs (v_rec_params.(poffset) + 768) = 768). *)
(*   Hint InitRely init_rec_regs (stack_ptr_extract_ofs (v_rec_params.(poffset) + 776) = 776). *)
(*   Hint InitRely init_rec_regs (stack_ptr_extract_ofs (v_rec_params.(poffset) + 784) = 784). *)
(*   Hint InitRely init_rec_regs (stack_ptr_extract_ofs (v_rec_params.(poffset) + 792) = 792). *)
(*   Hint InitRely init_rec_regs (stack_ptr_extract_ofs (v_rec_params.(poffset) + 800) = 800). *)
(*   Hint InitRely init_rec_regs ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_rec_params.(poffset) + 768))).(stack_data_type) = StackDataTypeRecParams). *)
(*   Hint InitRely init_rec_regs ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_rec_params.(poffset) + 776))).(stack_data_type) = StackDataTypeRecParams). *)
(*   Hint InitRely init_rec_regs ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_rec_params.(poffset) + 784))).(stack_data_type) = StackDataTypeRecParams). *)
(*   Hint InitRely init_rec_regs ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_rec_params.(poffset) + 792))).(stack_data_type) = StackDataTypeRecParams). *)
(*   Hint InitRely init_rec_regs ((st.(priv).(pcpu_stack_data_slots) @ (stack_ptr_extract_slot (v_rec_params.(poffset) + 800))).(stack_data_type) = StackDataTypeRecParams). *)
(*   Hint InitRely init_rec_regs (v_rd.(pbase) = "slot_rd"). *)
(*   Hint InitRely init_rec_regs (v_rd.(poffset) = 0). *)

(*   Hint NoTrans free_rec_aux_granules. *)
(* End InitRec. *)
...
(*   Definition LAYER_PTR_LTB : string := "ptr_ltb". *)
(*   Definition LAYER_PRIMS : list string := *)
(*     "handle_rsi_host_call" :: *)
(*       "handle_rsi_ipa_state_set" :: *)
(*       "check_pending_irq" :: *)
(*       "emulate_stage2_data_abort" :: *)
(*       "return_result_to_realm" :: *)
(*       (* Problems, moved to bottom *) *)
(*       (* "complete_rsi_host_call" :: *) *)
...
(*       nil. *)
(*   (* Need fill in: *) *)
(*   Definition handle_sysreg_access_trap_funptr_wrap301 (func_ptr: Ptr) (arg0: Ptr) (arg1: Ptr) (arg2: Z) (st: RData) : (option (bool * RData)) := *)
(*     None. *)
...
(* (* Problems due to complete_rsi_host_call *) *)
(* Section ExitHandler. *)
(*   Definition LAYER_DATA := RData. *)
(*   Definition LAYER_CODE : string := "./rmm.json". *)
(*   Definition LAYER_LOAD : string := "load_RData". *)
...
(*       nil. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap264 (func_ptr: Ptr) (arg0: Z) (arg1: Z) (arg2: Z) (arg3: Ptr) (st: RData) : (option RData) := *)
(*     None. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap243 (func_ptr: Ptr) (st: RData) : (option (Z * RData)) := *)
(*     None. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap246 (func_ptr: Ptr) (arg0: Z) (st: RData) : (option (Z * RData)) := *)
(*     None. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap249 (func_ptr: Ptr) (arg0: Z) (arg1: Z) (st: RData) : (option (Z * RData)) := *)
(*     None. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap252 (func_ptr: Ptr) (arg0: Z) (arg1: Z) (arg2: Z) (st: RData) : (option (Z * RData)) := *)
(*     None. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap255 (func_ptr: Ptr) (arg0: Z) (arg1: Z) (arg2: Z) (arg3: Z) (st: RData) : (option (Z * RData)) := *)
(*     None. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap258 (func_ptr: Ptr) (arg0: Z) (arg1: Z) (arg2: Z) (arg3: Z) (arg4: Z) (st: RData) : (option (Z * RData)) := *)
(*     None. *)
(*   (* Need fill in: *) *)
(*   Definition handle_ns_smc_funptr_wrap261 (func_ptr: Ptr) (arg0: Z) (arg1: Ptr) (st: RData) : (option RData) := *)
(*     None. *)
(* End ExitHandler. *)
