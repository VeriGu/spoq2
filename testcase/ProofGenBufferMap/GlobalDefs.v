...
Section GlobalDefs.
            ...
            then (Some (gic_virt_feature_3, st))
              ...
              then (Some (gic_virt_feature_4, st))
                ...
                then (
                  ...
                  rely (((s_granule_field_accessible (((st.(share)).(granules)) @ idx) ofs) = (true)));
  ...
  Definition store_RData (sz: Z) (p: Ptr) (v: Z) (st: RData) : (option RData) :=
    ...
    then (
      ...
      rely (((s_granule_field_accessible (((st.(share)).(granules)) @ idx) elem_ofs) = (true)));
        ...
        then (Some SLOT_RD)
          ...
          then (Some SLOT_REC)
            ...
            then (Some SLOT_REC2)
              ...
              then (Some SLOT_REC_TARGET)
              else (
                if (b =s ("slot_rec_aux0"))
                then (Some SLOT_REC_AUX0)
                else (
                  if (b =s ("slot_rtt"))
                  then (Some SLOT_RTT)
                  else (
                    ...
                    then (Some SLOT_RTT2)
                      ...
                      then (Some SLOT_RSI_CALL)
                      else (
                        if (b =s ("attest_setup_platform_token_stack"))
                        then (Some STACK_attest_setup_platform_token)
                        else (
                          if (b =s ("smc_psci_complete_stack"))
                          then (Some STACK_smc_psci_complete)
                          else (
                            if (b =s ("find_lock_two_granules_stack"))
                            then (Some STACK_find_lock_two_granules)
                            else (
                              if (b =s ("attest_token_continue_write_state_stack"))
                              then (Some STACK_attest_token_continue_write_state)
                              else (
                                if (b =s ("rmm_log_stack"))
                                then (Some STACK_rmm_log)
                                else (
                                  if (b =s ("attest_realm_token_create_stack"))
                                  then (Some STACK_attest_realm_token_create)
                                  else (
                                    if (b =s ("smc_rec_enter_stack"))
                                    then (Some STACK_smc_rec_enter)
                                    else (
                                      if (b =s ("do_host_call_stack"))
                                      then (Some STACK_do_host_call)
                                      else (
                                        if (b =s ("attest_rnd_prng_init_stack"))
                                        then (Some STACK_attest_rnd_prng_init)
                                        else (
                                          if (b =s ("plat_setup_stack"))
                                          then (Some STACK_plat_setup)
                                          else (
                                            if (b =s ("attest_token_encode_start_stack"))
                                            then (Some STACK_attest_token_encode_start)
                                            else (
                                              if (b =s ("smc_data_destroy_stack"))
                                              then (Some STACK_smc_data_destroy)
                                              else (
                                                if (b =s ("xlat_get_llt_from_va_stack"))
                                                then (Some STACK_xlat_get_llt_from_va)
                                                else (
                                                  if (b =s ("smc_rec_create_stack"))
                                                  then (Some STACK_smc_rec_create)
                                                  else (
                                                    if (b =s ("measurement_extend_sha512_stack"))
                                                    then (Some STACK_measurement_extend_sha512)
                                                    else (
                                                      if (b =s ("data_granule_measure_stack"))
                                                      then (Some STACK_data_granule_measure)
                                                      else (
                                                        if (b =s ("sort_granules_stack"))
                                                        then (Some STACK_sort_granules)
                                                        else (
                                                          if (b =s ("measurement_extend_sha256_stack"))
                                                          then (Some STACK_measurement_extend_sha256)
                                                          else (
                                                            if (b =s ("realm_ipa_to_pa_stack"))
                                                            then (Some STACK_realm_ipa_to_pa)
                                                            else (
                                                              if (b =s ("attest_realm_token_sign_stack"))
                                                              then (Some STACK_attest_realm_token_sign)
                                                              else (
                                                                if (b =s ("rmm_el3_ifc_get_platform_token_stack"))
                                                                then (Some STACK_rmm_el3_ifc_get_platform_token)
                                                                else (
                                                                  if (b =s ("attest_init_realm_attestation_key_stack"))
                                                                  then (Some STACK_attest_init_realm_attestation_key)
                                                                  else (
                                                                    if (b =s ("plat_cmn_setup_stack"))
                                                                    then (Some STACK_plat_cmn_setup)
                                                                    else (
                                                                      if (b =s ("complete_rsi_host_call_stack"))
                                                                      then (Some STACK_complete_rsi_host_call)
                                                                      else (
                                                                        if (b =s ("handle_realm_rsi_stack"))
                                                                        then (Some STACK_handle_realm_rsi)
                                                                        else (
                                                                          if (b =s ("smc_rtt_set_ripas_stack"))
                                                                          then (Some STACK_smc_rtt_set_ripas)
                                                                          else (
                                                                            if (b =s ("rtt_walk_lock_unlock_stack"))
                                                                            then (Some STACK_rtt_walk_lock_unlock)
                                                                            else (
                                                                              if (b =s ("smc_rtt_destroy_stack"))
                                                                              then (Some STACK_smc_rtt_destroy)
                                                                              else (
                                                                                if (b =s ("map_unmap_ns_stack"))
                                                                                then (Some STACK_map_unmap_ns)
                                                                                else (
                                                                                  if (b =s ("handle_rsi_attest_token_init_stack"))
                                                                                  then (Some STACK_handle_rsi_attest_token_init)
                                                                                  else (
                                                                                    if (b =s ("realm_params_measure_stack"))
                                                                                    then (Some STACK_realm_params_measure)
                                                                                    else (
                                                                                      if (b =s ("handle_rsi_ipa_state_get_stack"))
                                                                                      then (Some STACK_handle_rsi_ipa_state_get)
                                                                                      else (
                                                                                        if (b =s ("realm_ipa_get_ripas_stack"))
                                                                                        then (Some STACK_realm_ipa_get_ripas)
                                                                                        else (
                                                                                          if (b =s ("smc_rtt_fold_stack"))
                                                                                          then (Some STACK_smc_rtt_fold)
                                                                                          else (
                                                                                            if (b =s ("smc_rtt_create_stack"))
                                                                                            then (Some STACK_smc_rtt_create)
                                                                                            else (
                                                                                              if (b =s ("rsi_log_on_exit_stack"))
                                                                                              then (Some STACK_rsi_log_on_exit)
                                                                                              else (
                                                                                                if (b =s ("attest_cca_token_create_stack"))
                                                                                                then (Some STACK_attest_cca_token_create)
                                                                                                else (
                                                                                                  if (b =s ("rec_params_measure_stack"))
                                                                                                  then (Some STACK_rec_params_measure)
                                                                                                  else (
                                                                                                    if (b =s ("handle_ns_smc_stack"))
                                                                                                    then (Some STACK_handle_ns_smc)
                                                                                                    else (
                                                                                                      if (b =s ("rmm_el3_ifc_get_realm_attest_key_stack"))
                                                                                                      then (Some STACK_rmm_el3_ifc_get_realm_attest_key)
                                                                                                      else (
                                                                                                        if (b =s ("handle_rsi_realm_config_stack"))
                                                                                                        then (Some STACK_handle_rsi_realm_config)
                                                                                                        else (
                                                                                                          if (b =s ("smc_rtt_init_ripas_stack"))
                                                                                                          then (Some STACK_smc_rtt_init_ripas)
                                                                                                          else (
                                                                                                            if (b =s ("smc_rtt_read_entry_stack"))
                                                                                                            then (Some STACK_smc_rtt_read_entry)
                                                                                                            else (
                                                                                                              if (b =s ("handle_data_abort_stack"))
                                                                                                              then (Some STACK_handle_data_abort)
                                                                                                              else (
                                                                                                                if (b =s ("data_create_stack"))
                                                                                                                then (Some STACK_data_create)
                                                                                                                else (
                                                                                                                  if (b =s ("smc_realm_create_stack"))
                                                                                                                  then (Some STACK_smc_realm_create)
                                                                                                                  else (
                                                                                                                    if (b =s ("ripas_granule_measure_stack"))
                                                                                                                    then (Some STACK_ripas_granule_measure)
                                                                                                                    else (
                                                                                                                      if (b =s ("ipa_is_empty_stack"))
                                                                                                                      then (Some STACK_ipa_is_empty)
                                                                                                                      else None))))))))))))))))))))))))))))))))))))))))))))))))))))))))).
  ...
  Definition int_to_ptr (v: Z) : Ptr :=
    if (v >? (0))
    then (
      if ((v >=? (GRANULES_BASE)) && ((v <? ((GRANULES_BASE + ((RMM_MAX_GRANULES * (ST_GRANULE_SIZE))))))))
