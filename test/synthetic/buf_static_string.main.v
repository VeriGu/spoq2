...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "buf_static_string".
Definition PROJ_BASE: string := "buf_static_string".
Definition PROJ_BC_PATH: string := "buf_static_string.bc".
...
Definition load_stack (sz: intSizeT) (p: Ptr) (stack_map: STACK): (option int64) :=
	match (stack_map @ p.(pbase)) with
		| Some sv => match sv with
			| ZVal ZVal_val => rely (false \/ (p.(pbase) =s "stack_type_1") \/ (p.(pbase) =s "stack_type_2"));
				Some ZVal_val
		end
	| None => None
end. (* load_stack *)
...
Definition load_global (sz: intSizeT) (p: Ptr) (st: RData) : (option int64) :=
  if (p.(pbase) =s "string") then (
    if (global_in_bounds sz p SZ_string) then (
      let idx := p.(poffset) / 1 in
      let ptr := st.(globals).(g_string) @ idx in
      Some(ptr)
    ) else None) else
  None. (* load_global *)
...
Definition store_global (sz: intSizeT) (p: Ptr) (v: int64) (st: RData) : option RData :=
  if (p.(pbase) =s "string") then (
    if (global_in_bounds sz p SZ_string) then (
      let idx := p.(poffset) / 1 in
      let ptr := (st.(globals).(g_string) # idx == v) in
      Some(st.[globals].[g_string] :< ptr)
    ) else None) else
  None. (* store_global *)

Definition STRING_BASE : intSizeT := 67108864.
...
Definition global_to_ptr (v: intSizeT) : Ptr :=
 ...
 if (v >=? STRING_BASE) then (mkPtr "string" (v - STRING_BASE)) else
   (mkPtr "null" 0).
Definition ptr_to_int (p: Ptr) : intSizeT :=
 if (p.(poffset) <? 0) then (-1) else
 if (p.(pbase) =s "null") then 0 else
 if (p.(pbase) =s "string") then (STRING_BASE + p.(poffset)) else
    (-1).
