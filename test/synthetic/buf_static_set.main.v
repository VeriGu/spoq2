...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "buf_static_set".
Definition PROJ_BASE: string := "buf_static_set".
Definition PROJ_BC_PATH: string := "buf_static_set.bc".


Hint StackVar patch v_idx.addr stack_type_2.
Hint StackVar patch v_value.addr stack_type_1.
Hint StackVar vuln v_idx.addr stack_type_2.
Hint StackVar vuln v_value.addr stack_type_1.
...
Definition load_global (sz: intSizeT) (p: Ptr) (st: RData) : (option int64) :=
  if (p.(pbase) =s "buf") then (
    if (global_in_bounds sz p SZ_buf) then (
      let idx := p.(poffset) / 1 in
      let ptr := st.(globals).(g_buf) @ idx in
      Some(ptr)
    ) else None) else
  None. (* load_global *)
...
Definition store_global (sz: intSizeT) (p: Ptr) (v: int64) (st: RData) : option RData :=
  if (p.(pbase) =s "buf") then (
    if (global_in_bounds sz p SZ_buf) then (
      let idx := p.(poffset) / 1 in
      let ptr := (st.(globals).(g_buf) # idx == v) in
      Some(st.[globals].[g_buf] :< ptr)
    ) else None) else
  None. (* store_global *)

Definition BUF_BASE : intSizeT := 67108864.
