...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "struct_basic".
Definition PROJ_BASE: string := "struct_basic".
Definition PROJ_BC_PATH: string := "struct_basic.bc".
...
Definition load_s_Point (sz: intSizeT) (ofs:intSizeT)  (st_s_Point: s_Point) : option int64 :=
  if (ofs =? 0) then Some (st_s_Point.(e_s_Point_0)) else
  if (ofs =? 4) then Some (st_s_Point.(e_s_Point_1)) else
  if (ofs =? 8) then Some (st_s_Point.(e_s_Point_2)) else
  None.

Definition store_s_Point (sz: intSizeT) (ofs:intSizeT) (v: int64)  (st: s_Point) : option s_Point :=
  if (ofs =? 0) then Some (st.[e_s_Point_0] :< v) else
  if (ofs =? 4) then Some (st.[e_s_Point_1] :< v) else
  if (ofs =? 8) then Some (st.[e_s_Point_2] :< v) else
  None.



Hint StackVar entry_vuln v_x.addr stack_type_1.
Hint StackVar entry_vuln v_p stack_s_Point.
Hint StackVar entry_patch v_x.addr stack_type_1.
Hint StackVar entry_patch v_p stack_s_Point.


Definition is_stack_ptr (p: Ptr): bool := (false = true) \/
	p.(pbase) =s "stack_type_1" \/
	p.(pbase) =s "stack_s_Point".


Inductive StackVal :=
	| ZMapVal (ZMapValConstr: (ZMap.t (ZMap.t int64)))
	| ZVal (ZValConstr: int64)
	| s_PointVal (s_PointValConstr: s_Point)
.
...
Definition load_stack (sz: intSizeT) (p: Ptr) (stack_map: STACK): (option int64) :=
	match (stack_map @ p.(pbase)) with
		| Some sv => match sv with
			| ZVal ZVal_val => rely (false \/ (p.(pbase) =s "stack_type_1"));
				Some ZVal_val
			| s_PointVal s_PointVal_val =>
				rely (false \/ (p.(pbase) =s "stack_s_Point"));
				when ret_load_s_PointVal == load_s_Point sz p.(poffset) s_PointVal_val;
					Some(ret_load_s_PointVal)
		end
	| None => None
end. (* load_stack *)
Definition store_stack (sz: intSizeT) (p: Ptr) (v: int64) (stack_map: STACK): (option STACK) :=
	match (stack_map @ p.(pbase)) with
		| Some sv => match sv with
			...
			| s_PointVal s_PointVal_val =>
				rely (false \/ (p.(pbase) =s "stack_s_Point"));
				when ret_store_s_PointVal == store_s_Point sz p.(poffset) v s_PointVal_val;
				Some(stack_map # p.(pbase) == Some(s_PointVal ret_store_s_PointVal))
		end
	| None => None
end. (* store_stack *)
Record GLOBALS :=
...
Definition SZ_point : intSizeT := 12.
...
Definition load_global (sz: intSizeT) (p: Ptr) (st: RData) : (option int64) :=
  if (p.(pbase) =s "point") then (
    if (global_in_bounds sz p SZ_point) then (
      when ret == load_s_Point sz p.(poffset) st.(globals).(g_point);
      Some(ret)
    ) else None) else
  None. (* load_global *)
...
Definition store_global (sz: intSizeT) (p: Ptr) (v: int64) (st: RData) : option RData :=
  if (p.(pbase) =s "point") then (
    if (global_in_bounds sz p SZ_point) then (
      when ret == store_s_Point sz p.(poffset) v st.(globals).(g_point);
      Some(st.[globals].[g_point] :< ret)
    ) else None) else
  None. (* store_global *)

Definition POINT_BASE : intSizeT := 67108864.
...
Definition global_to_ptr (v: intSizeT) : Ptr :=
 ...
 if (v >=? POINT_BASE) then (mkPtr "point" (v - POINT_BASE)) else
   (mkPtr "null" 0).
Definition ptr_to_int (p: Ptr) : intSizeT :=
 if (p.(poffset) <? 0) then (-1) else
 if (p.(pbase) =s "null") then 0 else
 if (p.(pbase) =s "point") then (POINT_BASE + p.(poffset)) else
    (-1).
Definition patch_correct_postcondition (st: RData) (st_sim: RData): Prop :=
	st.(globals).(g_point) = st_sim.(globals).(g_point).
Definition patch_correct_precondition (st: RData) (st_sim: RData): Prop :=
st = st_sim.

Hint Refines entry_vuln_spec entry_patch_spec patch_correct_precondition patch_correct_postcondition (vuln_ret = patch_ret).
Parameter llvm_memcpy_p0_p0_i64_spec : (Ptr-> (Ptr-> (int64-> (Z-> (RData-> (option RData)))))).
Parameter process_point_spec : (Ptr-> (RData-> (option RData))).
...
Section Bottom.
    ...
    Definition LAYER_PRIMS: list string :=
        "exit"::
        "process_point"::
        "llvm_memcpy_p0_p0_i64"::
        "llvm_lifetime_end_p0"::
        "malloc"::
        "llvm_lifetime_start_p0"::
        nil.
End Bottom.
