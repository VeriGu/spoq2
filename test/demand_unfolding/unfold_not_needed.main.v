...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "unfold_not_needed".
Definition PROJ_BASE: string := "unfold_not_needed".
Definition PROJ_BC_PATH: string := "unfold_not_needed.bc".
...
Definition load_global (sz: Z) (p: Ptr) (st: RData) : (option Z) :=
  ...
  if (p.(pbase) =s "g2") then (
      Some(st.(globals).(g_g2))) else
  None. (* load_global *)
