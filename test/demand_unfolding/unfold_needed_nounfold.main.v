...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "unfold_needed_nounfold".
Definition PROJ_BASE: string := "unfold_needed_nounfold".
Definition PROJ_BC_PATH: string := "unfold_needed_nounfold.bc".
...
(* The hint under test.  helper_spec is exactly what the demand-driven retry
   would inline to make this refinement go through -- see unfold_needed.main.v,
   which is this file without the hint and verifies.  With the hint the retry
   must leave it folded, so the proof is expected to fail and helper_spec must
   still appear as a call in the emitted spec. *)
