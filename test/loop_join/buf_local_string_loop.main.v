...
Parameter g___const_vuln_string: (ZMap.t Z).
Parameter g___const_patch_string: (ZMap.t Z).
Record GLOBALS :=
  mkGLOBALS {
      g_dumb: Z
    }.
...
Definition __CONST_PATCH_STRING_BASE : Z := 67108864.
Definition __CONST_VULN_STRING_BASE : Z := 67112960.
Definition DUMB_BASE : Z := 67117056.
Definition MAX_GLOBAL : Z := 67121152.
...
(* `int ext();` -- the opaque external call the check_matching_* samples use.
   ...
   spec_has_ub: false rather than true), replace this with:
     Parameter ext_ret : (RData -> (Z)).
     Definition ext_spec (st: RData) : (option ((Z) * RData)) :=
       Some ((ext_ret st), st). *)
Parameter ext_spec : (RData -> (option ((Z) * RData))).
