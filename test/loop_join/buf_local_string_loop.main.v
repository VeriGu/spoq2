...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "buf_local_string_loop".
Definition PROJ_BASE: string := "buf_local_string_loop".
Definition PROJ_BC_PATH: string := "buf_local_string_loop.bc".


Hint StackVar patch v_retval stack_type_1.
Hint StackVar patch v_idx.addr stack_type_2.
Hint StackVar patch v_string stack_type_3.
Hint StackVar patch v_i stack_type_2__1.
Hint StackVar patch v_tmp stack_type_1__1.
Hint StackVar patch v_cleanup.dest.slot stack_type_2__2.
Hint StackVar vuln v_idx.addr stack_type_2.
Hint StackVar vuln v_string stack_type_3.
Hint StackVar vuln v_i stack_type_2__1.
Hint StackVar vuln v_tmp stack_type_1.
...
Definition is_stack_ptr (p: Ptr): bool := (false = true) \/
	...
	p.(pbase) =s "stack_type_2__2" \/
	p.(pbase) =s "stack_type_3".
...
Definition load_stack (sz: intSizeT) (p: Ptr) (stack_map: STACK): (option int64) :=
	match (stack_map @ p.(pbase)) with
		| Some sv => match sv with
			| ZVal ZVal_val => rely (false \/ (p.(pbase) =s "stack_type_1") \/ (p.(pbase) =s "stack_type_1__1") \/ (p.(pbase) =s "stack_type_2") \/ (p.(pbase) =s "stack_type_2__1") \/ (p.(pbase) =s "stack_type_2__2"));
				Some ZVal_val
		end
	| None => None
end. (* load_stack *)
...
Parameter g___const_vuln_string: (ZMap.t int64).
Parameter g___const_patch_string: (ZMap.t int64).
Record GLOBALS :=
  mkGLOBALS {
      g_dumb: int64
    }.
...
Definition is_global_ptr (p: Ptr): bool := (false = true)\/
	"__const_patch_string" =s p.(pbase)\/
	"__const_vuln_string" =s p.(pbase)\/
	"dumb" =s p.(pbase).

Definition load_global (sz: intSizeT) (p: Ptr) (st: RData) : (option int64) :=
  if (p.(pbase) =s "__const_patch_string") then (
      let idx := p.(poffset) / 1 in
      let ptr := g___const_patch_string @ idx in
      Some(ptr)) else
  if (p.(pbase) =s "__const_vuln_string") then (
      let idx := p.(poffset) / 1 in
      let ptr := g___const_vuln_string @ idx in
      Some(ptr)) else
  if (p.(pbase) =s "dumb") then (
      Some(st.(globals).(g_dumb))) else
  None. (* load_global *)

Definition store_global (sz: intSizeT) (p: Ptr) (v: int64) (st: RData) : option RData :=
  if (p.(pbase) =s "dumb") then (
      Some(st.[globals].[g_dumb] :< v)) else
  None. (* store_global *)

Definition __CONST_PATCH_STRING_BASE : intSizeT := 67108864.
Definition __CONST_VULN_STRING_BASE : intSizeT := 67112960.
Definition DUMB_BASE : intSizeT := 67117056.
Definition MAX_GLOBAL : intSizeT := 67121152.
...
Definition global_to_ptr (v: intSizeT) : Ptr :=
 ...
 if (v >=? __CONST_VULN_STRING_BASE) then (mkPtr "__const_vuln_string" (v - __CONST_VULN_STRING_BASE)) else
 if (v >=? __CONST_PATCH_STRING_BASE) then (mkPtr "__const_patch_string" (v - __CONST_PATCH_STRING_BASE)) else
   (mkPtr "null" 0).
Definition ptr_to_int (p: Ptr) : intSizeT :=
 ...
 if (p.(pbase) =s "__const_vuln_string") then (__CONST_VULN_STRING_BASE + p.(poffset)) else
 if (p.(pbase) =s "__const_patch_string") then (__CONST_PATCH_STRING_BASE + p.(poffset)) else
    (-1).
...
(* `int ext();` -- the opaque external call the check_matching_* samples use.
   clang emits `declare i32 @ext(...)` for the unprototyped declaration, and the
   samples call it with no arguments, so the spec takes only the state.
   A Parameter, not a Definition: an external function we have no body for may
   return anything, touch memory, or not return at all.  What matters for those
   samples is that both sides get the *same* unknown value from the same state,
   which leaves the refinement resting on the arithmetic around the call.
   spoq derives the spec body `(ext_spec st)` from the bitcode and needs only
   this type; without it, it aborts with `(z3_eval) Unknown symbol: ext_spec`.
   For a model where ext always returns and leaves the state alone (reports
   spec_has_ub: false rather than true), replace this with:
     Parameter ext_ret : (RData -> (int32)).
     Definition ext_spec (st: RData) : (option ((int32) * RData)) :=
       Some ((ext_ret st), st). *)
Parameter ext_spec : (RData -> (option ((int32) * RData))).
...
Definition patch_correct (st: RData) (st_sim: RData): Prop :=
    st = st_sim (* Replace this line with patch_correct relation *)
    .
