...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "attr_argmem_two_pointers".
Definition PROJ_BASE: string := "attr_argmem_two_pointers".
Definition PROJ_BC_PATH: string := "attr_argmem_two_pointers.bc".


Hint StackVar vuln v_1 stack_type_1.
Hint StackVar vuln v_2 stack_type_1__1.
Hint StackVar patch v_1 stack_type_1.
Hint StackVar patch v_2 stack_type_1__1.
...
Definition load_global (sz: intSizeT) (p: Ptr) (st: RData) : (option int64) :=
  if (p.(pbase) =s "g") then (
    if (global_in_bounds sz p SZ_g) then (
      Some(st.(globals).(g_g))
    ) else None) else
  None. (* load_global *)
...
Hint Refines vuln_spec patch_spec patch_correct_precondition patch_correct_postcondition (vuln_ret = patch_ret).
...
Definition ptr_ltb (p1: Ptr) (p2: Ptr) : bool := ptr_to_int p1 <? ptr_to_int p2.
Definition ptr_leb (p1: Ptr) (p2: Ptr) : bool := ptr_to_int p1 <=? ptr_to_int p2.
Definition ptr_gtb (p1: Ptr) (p2: Ptr) : bool := ptr_to_int p1 >? ptr_to_int p2.
Definition ptr_ugt (p1: Ptr) (p2: Ptr) : bool := ptr_to_int p1 >? ptr_to_int p2.
Definition spoq_zext_spec (v_0: bool) : Z := if (v_0) then 1 else 0.
Parameter ext_argmem_two_pointers_spec : (Ptr-> (Ptr-> (RData-> (option ((int32) * RData))))).
