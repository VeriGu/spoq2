...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "int_zext_i8_range".
Definition PROJ_BASE: string := "int_zext_i8_range".
Definition PROJ_BC_PATH: string := "int_zext_i8_range.bc".
...
(* An i8 result: one of 256 bit patterns, so its zero extension is 0..255. *)
Parameter pick8_spec : (RData-> (option ((int8) * RData))).
...
Hint Postcondition vuln_spec (_ret_0 <? (256)).
