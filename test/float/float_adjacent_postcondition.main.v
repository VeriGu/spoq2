...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "float_adjacent_postcondition".
Definition PROJ_BASE: string := "float_adjacent_postcondition".
Definition PROJ_BC_PATH: string := "float_adjacent_postcondition.bc".
...
(* The return value is bounded regardless of what the floats do: `sel` is 0 or
   1 from the integer comparison, and `k` is 0 or 1 from the float one, so the
   sum is between 0 and 2 on every path.

   The float branch cannot be decided -- fcmp_ogt is uninterpreted -- so both
   arms stay feasible and the property has to hold on both.  That is the point
   of the case: a property next to floats, proved without modelling them.

   Both bounds are true on all paths, deliberately: check_pre_post currently
   proves only the `then` branch, so a one-sided postcondition would pass here
   vacuously and start failing once that is fixed.

   CheckInv is what schedules the proof; without it the hints parse and are
   never proved. *)
Hint CheckInv vuln_spec.
Hint Postcondition vuln_spec (_ret_0 >=? (0)).
Hint Postcondition vuln_spec (_ret_0 <=? (2)).
