...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "callee_in_higher_layer".
Definition PROJ_BASE: string := "callee_in_higher_layer".
Definition PROJ_BC_PATH: string := "callee_in_higher_layer.bc".
...
(* `helper` sits below its callers, as it should.  `helper2` sits *above* them,
   so `vuln_spec_low` is inferred while `helper2_spec` does not yet exist.  That
   is the arrangement ffm001 has, where `sws_getContext_vuln` is in Layer1 and
   the `sws_init_context_vuln` it calls is in Layer5. *)
