...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "nd_array_global_read".
Definition PROJ_BASE: string := "nd_array_global_read".
Definition PROJ_BC_PATH: string := "nd_array_global_read.bc".
...
Parameter g_mask: (ZMap.t (ZMap.t (ZMap.t int64))).
Record GLOBALS :=
  mkGLOBALS {
...
Definition SZ_mask : intSizeT := 72.
...
Definition MASK_BASE : intSizeT := 67108864.
...
Definition global_to_ptr (v: intSizeT) : Ptr :=
 ...
 if (v >=? MASK_BASE) then (mkPtr "mask" (v - MASK_BASE)) else
   (mkPtr "null" 0).
