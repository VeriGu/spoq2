...
(* Hint OnlyTrans smc_realm_destroy. *)
(* Hint CheckInv smc_realm_destroy_spec. *)

(* Hint OnlyTrans smc_realm_activate. *)
(* Hint CheckInv smc_realm_activate_spec. *)
(* Hint OnlyTrans smc_rtt_map_unprotected. *)
(* Hint CheckInv smc_rtt_map_unprotected_spec. *)
...
Definition load_s_granule (sz: Z) (ofs: Z) (sg: s_granule) : option Z :=
  ...
  if (ofs =? 4) then Some sg.(e_state)
  else
    (* RD's refcount can be lock-free accessed, but must be atomic *)
    if (ofs =? 8) then Some (sg.(e_refcount))
    else
      None.
...
Definition store_RData (sz: Z) (p: Ptr) (v: Z) (st: RData) : option RData :=
  if p.(pbase) =s "granules" then (
      ...
      if (elem_ofs =? 8) && ((st.(share).(granules) @ idx).(e_state) =? GRANULE_STATE_REC) then
...
Section Bottom.
  ...
  Definition memcpy_ns_read_spec (v_dest: Ptr) (v_src: Ptr) (v_conv: Z) (st: RData) : option (bool * RData) :=  Some (true, st).
  ...
  Definition memset_spec (v_s: Ptr) (c: Z) (n: Z) (st: RData) : option (Ptr * RData) :=
    ...
    else if ((v_s.(pbase) =s "slot_rtt")) then
      Some (st.(share).(slots) @ SLOT_RTT)
...
Section TableWalk.
  ...
  (
  forall (rd: Z) (rd2: Z) (rtt_idx: Z), 
  (((st.(share).(granules) @ rd).(e_state) = GRANULE_STATE_RD) -> 
  ((st.(share).(granules) @ rd2).(e_state) = GRANULE_STATE_RD) -> 
  rd_rtt (st.(share)) rd = rtt_idx -> 
  rd_rtt (st.(share)) rd2 = rtt_idx -> 
  ((st.(share).(granules) @ rtt_idx).(e_state) = GRANULE_STATE_RTT) -> 
  (rd = rd2))
  ). *)
...
Section RealmCreate.
  ...
  Loop_inv total_root_rtt_refcount_loop348
  ((v_indvars_iv >= 0) /\ (v_indvars_iv < v_wide_trip_count) /\ (v_wide_trip_count > 0)).
  ...
  (* Loop_inv free_sl_rtts_loop193 
  (
    (0 <= v_indvars_iv) /\ (v_indvars_iv < v_wide_trip_count) /\ (v_wide_trip_count > 0)
  ). *)
  ...
  (
   ...
   (* satisfy rd_rtt_reverse *)
  (* Loop_inv free_sl_rtts_loop193
  (
  forall (rd: Z) (rd2: Z) (rtt_idx: Z), 
  (((st.(share).(granules) @ rd).(e_state) = GRANULE_STATE_RD) ->
  ((st.(share).(granules) @ rd2).(e_state) = GRANULE_STATE_RD) ->
  addr_to_gidx (rd_rtt (st.(share)) rd) = rtt_idx -> 
  addr_to_gidx (rd_rtt (st.(share)) rd2) = rtt_idx ->
  ((st.(share).(granules) @ rtt_idx).(e_state) = GRANULE_STATE_RTT) ->
  (rd = rd2))
  ). *)
  ...
  Loop_inv free_sl_rtts_loop193
  (
    ((st_old.(share).(granules) @ gidx).(e_state) <> GRANULE_STATE_RTT ->
      (st.(share).(granules) @ gidx = (st_old).(share).(granules) @ gidx /\
      st.(share).(granule_data) @ gidx = (st_old).(share).(granule_data) @ gidx)
    )
  ).

  Loop_inv free_sl_rtts_loop193
  (
    forall (gidx: Z), (st.(share).(gpt) = st_old.(share).(gpt))
  ).

  (* satisfy delegate zero *)
  (* Loop_inv free_sl_rtts_loop193
  (
    forall (gidx: Z),
  ...
  (* Loop_inv free_sl_rtts_loop193 
  (
    forall (i:Z), (((0 <= i) /\ (i < v_indvars_iv)) -> 
    st.(share).(granule_data) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE)
    = ((((st_old.(share).(granule_data) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE)).[g_norm] :< zero_granule_data_normal).[g_rec] :< empty_rec).[g_rd] :< empty_rd))
  ).

  Loop_inv free_sl_rtts_loop193
  (
    forall (i:Z), (((i >= v_indvars_iv) /\ (i < v_wide_trip_count)) -> 
    ((((__break__ = false)) ->
    st.(share).(granule_data) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE) = st_old.(share).(granule_data) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE))
    /\ 
    (((__break__ = true)) ->
    st.(share).(granule_data) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE) = ((((st_old.(share).(granule_data) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE)).[g_norm] :< zero_granule_data_normal).[g_rec] :< empty_rec).[g_rd] :< empty_rd))
    ))
  ).
  ...
  (
    forall (i:Z), (((0 <= i) /\ (i < v_indvars_iv)) -> 
     st.(share).(granules) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE) =
     (((st_old.(share).(granules) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE)).[e_lock] :< None).[e_state] :< 1))
  ). 
  ...
  (
    forall (i:Z), (((i >= v_indvars_iv) /\ (i < v_wide_trip_count)) -> 
    ((((__break__ = false)) ->
    st.(share).(granules) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE) = st_old.(share).(granules) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE))
    /\ 
    (((__break__ = true)) ->
    st.(share).(granules) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE) = (((st_old.(share).(granules) @ ((v_g_rtt.(poffset) + 16 * (i)) / ST_GRANULE_SIZE)).[e_lock] :< None).[e_state] :< 1))))
  ). *)
...
End SMCHandler.
Hint Preserve query_oracle.
Hint Postcondition query_oracle ((st.(share).(gpt) = st_old.(share).(gpt)) /\ (st.(repl) = st_old.(repl)) /\ (st.(priv) = st_old.(priv)) /\ (st.(stack) = st_old.(stack))).

(* guest frame number to rtt offset *)
Parameter gfn_to_rtt_offs: Z -> (Z -> (Z)).
(* guest frame number to pte offset *)
Parameter gfn_to_pte_offs: Z -> (Z -> (Z)).
Parameter ADDR_MASK: Z.
Parameter PTE_IS_VALID: Z -> (bool).
Parameter PTE_IS_TABLE: Z -> (bool).

(* struct granule* addr to granule idx = (addr - GRANULE_BASE)/size *)
Parameter addr_to_gidx : Z -> Z.
(*rtt granule idx to pte*)
Parameter gidx_to_table_pte : Z -> (Z).
(* pte to granule index*)
Parameter pte_to_gidx: Z -> (Z).

Parameter PTE_PERM: Z -> (Z).

Axiom gidx_pte :
forall (idx:Z) (pte: Z) , gidx_to_table_pte idx = pte -> pte_to_gidx pte = idx.

Axiom pte_gidx :
forall (idx:Z) (pte:Z), pte_to_gidx pte = idx = pte -> gidx_to_table_pte idx.
...
Definition walk_rtt (sh: Shared) (rd: Z) (gfn:Z)  : (option Z) :=
  ...
  match walk_rtt_level (z_to_nat 4) sh 0 rd gfn rtt_pte with
  | (lvl', pte') =>
...
(* Definition walk_reverse (sh: Shared) : Prop :=
  ...
  (forall (rd: Z) (gfn: Z) (rtt_idx: Z)
      ...
      (forall (par_idx: Z) (ofs: Z), (rev rtt_idx = (par_idx, ofs) ->
                         (* if it is root rtt, the par_idx is rd*)
                      ((rtt_idx = rd_rtt sh rd -> par_idx = rd /\ ofs = 0)
                      (* if it is not root rtt, the par_idx is a pte page, ofs is pt offset*)
                       /\ (rtt_idx <> rd_rtt sh rd ->
                           ((sh.(granules) @ par_idx).(e_state) = GRANULE_STATE_RTT
                            /\ pte_to_gidx ((sh.(granule_data) @ par_idx).(g_norm) @ ofs) = rtt_idx)))))). *)

(* Invariant walk_reverse1 :=
  forall (rd: Z) (gfn: Z) (rtt_idx: Z),
    walk_star (st.(share)) rd gfn rtt_idx ->
    (st.(share).(granules) @ rtt_idx).(e_state) = GRANULE_STATE_RTT ->
    (forall (par_idx: Z) (par_idx2 : Z) (ofs: Z),
    (rtt_idx <> rd_rtt sh rd -> 
    (st.(share).(granules) @ par_idx).(e_state) = GRANULE_STATE_RTT ->
    (st.(share).(granules) @ par_idx2).(e_state) = GRANULE_STATE_RTT ->
     pte_to_gidx ((sh.(granule_data) @ par_idx1).(g_norm) @ ofs) = rtt_idx ->
     pte_to_gidx ((sh.(granule_data) @ par_idx2).(g_norm) @ ofs) = rtt_idx ->
     par_idx1 = par_idx2
    )). *)
...
(* a reverse map from root rtt_idx to rd index, each root rtt_idx is all maped to rd*)
(* note, this is second order formula, not solvable by z3*)
(* Invariant rd_rtt_reverse :=
  exists (rev: Z -> (Z)),
  (forall (rd: Z) (rtt_idx: Z), rd_rtt (st.(share)) rd = rtt_idx -> (rev rtt_idx = rd)). *)


(* Invariant rd_rtt_reverse1 :=
  forall (rd: Z) (rd2: Z) (rtt_idx: Z), 
  (((st.(share).(granules) @ rd).(e_state) = GRANULE_STATE_RD) ->
  ((st.(share).(granules) @ rd2).(e_state) = GRANULE_STATE_RD) ->
  addr_to_gidx (rd_rtt (st.(share)) rd) = rtt_idx -> 
...
Section Relations.
Definition relate_local (st: RData) (st': RData) : Prop :=
  st.(log) = st'.(log) /\
  st.(oracle) = st'.(oracle) /\
  st.(repl) = st'.(repl) /\
  st.(priv) = st'.(priv) /\
...
Definition relate_share_other (st: RData) (st': RData) : Prop :=
st.(share).(gpt) = st'.(share).(gpt) /\
st.(share).(glk) = st'.(share).(glk) /\
st.(share).(slots) = st'.(share).(slots) /\
st.(share).(granules) = st'.(share).(granules) /\
st.(share).(gv_g_sve_max_vq) = st'.(share).(gv_g_sve_max_vq) /\
st.(share).(gv_g_ns_simd) = st'.(share).(gv_g_ns_simd) /\
st.(share).(gv_g_cpu_simd_type) = st'.(share).(gv_g_cpu_simd_type) /\
st.(share).(gv_vmids) = st'.(share).(gv_vmids).

Definition relate_non_data (st: RData) (st': RData) : Prop :=
(forall (i : Z), ((st.(share).(granule_data) @ i).(gd_g_idx) = (st'.(share).(granule_data) @ i).(gd_g_idx))) /\
(forall (i : Z), ((st.(share).(granule_data) @ i).(g_rd) = (st'.(share).(granule_data) @ i).(g_rd))) /\
(forall (i : Z), ((st.(share).(granule_data) @ i).(g_rec) = (st'.(share).(granule_data) @ i).(g_rec))) /\
(forall (i : Z), ((st.(share).(granule_data) @ i).(g_aux_pmu_state) = (st'.(share).(granule_data) @ i).(g_aux_pmu_state))) /\
(forall (i : Z), ((st.(share).(granule_data) @ i).(g_aux_simd_state) = (st'.(share).(granule_data) @ i).(g_aux_simd_state))) /\
(forall (i : Z), ((st.(share).(granule_data) @ i).(rec_gidx) = (st'.(share).(granule_data) @ i).(rec_gidx))).

Definition relate_non_data_g (st: RData) (st': RData) : Prop :=
(forall (i : Z), ((st.(share).(granules) @ i).(e_state) <> GRANULE_STATE_DATA ->
    (st.(share).(granule_data) @ i).(g_norm) = (st'.(share).(granule_data) @ i).(g_norm))).

    
Record SecureRealmState := SecRm {
      sec_mem: ZMap.t (ZMap.t (option Z));
      sec_reg: ZMap.t (ZMap.t (option Z))
}.

Definition SecState := ZMap.t SecureRealmState.
Hint NoUnfold walk_rtt.
Definition relate_secure (norm_rdata: RData) (sec_rdata: RData) (sec_st: SecState) : Prop :=
  forall (rd_gidx: Z) (vaddr: Z) (data_gidx: Z),
  ((sec_rdata.(share).(granules) @ rd_gidx).(e_state) = GRANULE_STATE_RD ->
  (walk_rtt (sec_rdata.(share)) rd_gidx (addr_to_gidx vaddr) = Some data_gidx),
...
(* 
Definition relate_share (st: RData) (st': RData) := *)
