...
Parameter S2TTEDesc_init: RData.

Section S2TTEDesc_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition S2TTEDesc_newframe (fname: string) (st: RData) : option (RData) := None.

  Definition S2TTEDesc_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition S2TTEDesc_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition S2TTEDesc_get_flag (f: flag) (st: RData) : bool := false.

  Definition S2TTEDesc_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition S2TTEDesc_layer :=
    {|
      State := RData;
      Init := S2TTEDesc_init;
      Load := load_RData;
      Store := store_RData;
      NewFrame := S2TTEDesc_newframe;
      Alloca := alloc_stack;
      Free := free_stack;
      GetReg :=S2TTEDesc_get_reg;
      SetReg := S2TTEDesc_set_reg;
      GetFlag := S2TTEDesc_get_flag;
      SetFlag := S2TTEDesc_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
