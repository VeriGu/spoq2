...
(* SPOQ STOP *)
Definition PROJ_NAME: string := "global_bounds_refinement".
Definition PROJ_BASE: string := "global_bounds_refinement".
Definition PROJ_BC_PATH: string := "global_bounds_refinement.bc".
...
Record GLOBALS :=
  mkGLOBALS {
      g_table: (ZMap.t int64)
    }.
Record RData := mkRData {	stack: STACK;	heap: MEM;	globals: GLOBALS}.
Definition is_global_ptr (p: Ptr): bool := (false = true)\/
	"table" =s p.(pbase).

(* The extent of `table`, and the bound the refinement turns on: drop the
   global_in_bounds test below and the vuln reads off the end of the array
   with a defined value, which the patch's 0 does not match. *)
Definition SZ_table : intSizeT := 256.
...
Definition load_global (sz: intSizeT) (p: Ptr) (st: RData) : (option int64) :=
  if (p.(pbase) =s "table") then (
    if (global_in_bounds sz p SZ_table) then (
      let idx := p.(poffset) / 1 in
      let ptr := st.(globals).(g_table) @ idx in
      Some(ptr)
    ) else None) else
  None. (* load_global *)
...
Definition store_global (sz: intSizeT) (p: Ptr) (v: int64) (st: RData) : option RData :=
  if (p.(pbase) =s "table") then (
    if (global_in_bounds sz p SZ_table) then (
      let idx := p.(poffset) / 1 in
      let ptr := (st.(globals).(g_table) # idx == v) in
      Some(st.[globals].[g_table] :< ptr)
    ) else None) else
  None. (* store_global *)

Definition TABLE_BASE : intSizeT := 67108864.
...
Definition global_to_ptr (v: intSizeT) : Ptr :=
 ...
 if (v >=? TABLE_BASE) then (mkPtr "table" (v - TABLE_BASE)) else
   (mkPtr "null" 0).
Definition ptr_to_int (p: Ptr) : intSizeT :=
 if (p.(poffset) <? 0) then (-1) else
 if (p.(pbase) =s "null") then 0 else
 if (p.(pbase) =s "table") then (TABLE_BASE + p.(poffset)) else
    (-1).
