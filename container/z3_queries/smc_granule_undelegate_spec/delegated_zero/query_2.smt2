...
(assert
 ...
 (not $x564))))))))))))
(assert
 (let (($x1075 (forall ((gidx.186541 Int) )(let (($x54 (= (g_norm (select (granule_data (share st.0)) gidx.186541)) zero_granule_data)))
 (let (($x56 (= (g_granule_state (select (granule_data (share st.0)) gidx.186541)) 1)))
 (=> $x56 $x54))))
 ))
 (not $x1075)))
(check-sat)
