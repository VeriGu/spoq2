...
(assert
 ...
 (let (($x1515 (and $x1536 $x1510 $x1513)))
 (not $x1515)))))))
(assert
 (let (($x1596 (forall ((gidx.289074 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.289074)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.289074)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x1596)))
(check-sat)
