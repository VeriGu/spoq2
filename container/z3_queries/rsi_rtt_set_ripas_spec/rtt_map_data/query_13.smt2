...
(assert
 ...
 (not $x6488))))
(assert
 (let (($x6890 (forall ((rtt_gidx.38677916 Int) (ipa_gidx.38677916 Int) (data_gidx.38677916 Int) )(let ((?x13 (share st.0)))
 (let ((?x51 (granule_data ?x13)))
 (let ((?x138 (g_granule_state (select ?x51 (rtt_walk_abs.0_call st.0 rtt_gidx.38677916 ipa_gidx.38677916)))))
 (let (($x142 (rtt_is_root.0_call rtt_gidx.38677916)))
 (=> (and (= (e_state_s_granule (select (g_granules (globals ?x13)) rtt_gidx.38677916)) 5) $x142) (or (= ?x138 4) (= ?x138 6))))))))
 ))
 (not $x6890)))
(check-sat)
