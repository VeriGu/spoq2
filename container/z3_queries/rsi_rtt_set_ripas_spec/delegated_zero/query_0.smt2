...
(assert
 ...
 (not $x863)))
(assert
 (let (($x1047 (forall ((gidx.436806 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.436806)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.436806)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x1047)))
(check-sat)
