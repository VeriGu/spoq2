...
(assert
 ...
 (= ?x1309 5)))))))))
(assert
 (let (($x1247 (forall ((gidx.437167 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.437167)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.437167)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x1247)))
(check-sat)
