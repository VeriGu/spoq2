...
(assert
 ...
 (not $x1998)))))))))))
(assert
 (let (($x2409 (forall ((gidx.438674 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.438674)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.438674)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x2409)))
(check-sat)
