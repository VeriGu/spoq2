...
(assert
 ...
 (not $x1300))))))))))
(assert
 (let (($x1530 (forall ((gidx.437454 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.437454)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.437454)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x1530)))
(check-sat)
