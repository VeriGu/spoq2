...
(assert
 ...
 (not $x928))))))))))))
(assert
 (let (($x1250 (forall ((gidx.437130 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.437130)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.437130)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x1250)))
(check-sat)
