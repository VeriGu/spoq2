...
(assert
 ...
 (or (not $x912) $x915))))
(assert
 (forall ((v_0.107793 Int) )(let ((?x12 (share st.0)))
 (let ((?x91 (gpt ?x12)))
 (select ?x91 (div (meta_granule_offset (meta_PA (test_Z_PTE.0_call v_0.107793))) 4096)))))
 )
(assert
 (let (($x863 (<= v_3.0 1)))
 (let (($x864 (not $x863)))
...
(assert
 ...
 (= ?x999 5)))))))))))
(assert
 (let (($x1049 (forall ((gidx.436843 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.436843)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.436843)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x1049)))
(check-sat)
