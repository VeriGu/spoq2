...
(assert
 (let (($x17416 (forall ((gidx.6607894 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.6607894)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.6607894)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x17416)))
(check-sat)
