...
(assert
 (let (($x8432 (forall ((gidx.2466810 Int) )(let (($x8372 (= (g_norm (select (granule_data (share st.0)) gidx.2466810)) zero_granule_data)))
 (let (($x8326 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.2466810)) 1)))
 (=> $x8326 $x8372))))
 ))
 (not $x8432)))
(check-sat)
