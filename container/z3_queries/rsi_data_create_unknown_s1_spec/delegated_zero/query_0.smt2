...
(assert
 ...
 (or (not $x1113) $x1117))))
(assert
 (forall ((v_0.126244 Int) )(let ((?x12 (share st.0)))
 (let ((?x91 (gpt ?x12)))
 (select ?x91 (div (meta_granule_offset (meta_PA (test_Z_PTE.0_call v_0.126244))) 4096)))))
 )
(assert
 (let ((?x1105 (test_PA.0_call v_1.0)))
 (check_rcsm_mask_para.0_call ?x1105)))
(assert
 (let (($x1130 (forall ((gidx.2505014 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.2505014)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.2505014)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x1130)))
(check-sat)
