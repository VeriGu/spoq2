...
(assert
 ...
 (not $x2384))))))))))
(assert
 (let (($x37141 (forall ((gidx.2509659 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.2509659)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.2509659)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x37141)))
(check-sat)
