...
(assert
 ...
 (= ?x1233 0)))))))))
(assert
 (let (($x29675 (forall ((gidx.2509622 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.2509622)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.2509622)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x29675)))
(check-sat)
