...
(assert
 (let (($x896 (forall ((gidx.365707 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.365707)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.365707)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x896)))
(check-sat)
