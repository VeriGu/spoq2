...
(assert
 ...
 (not $x1595))))
(assert
 (let (($x3154 (forall ((gidx.369445 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.369445)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.369445)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x3154)))
(check-sat)
