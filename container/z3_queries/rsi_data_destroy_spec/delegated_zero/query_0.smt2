...
(assert
 ...
 (check_rcsm_mask_para.0_call ?x8333)))
(assert
 (let (($x64771 (forall ((gidx.145592069 Int) )(let (($x59 (= (g_norm (select (granule_data (share st.0)) gidx.145592069)) zero_granule_data)))
 (let (($x62 (= (e_state_s_granule (select (g_granules (globals (share st.0))) gidx.145592069)) 1)))
 (=> $x62 $x59))))
 ))
 (not $x64771)))
(check-sat)
