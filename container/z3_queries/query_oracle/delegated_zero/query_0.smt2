...
(assert
 (let (($x31308 (forall ((gidx.2347733 Int) )(let ((?x12 (share st.0)))
 (let ((?x7 (log st.0)))
 (let ((?x10 (|(oracle st.0)_call| ?x7)))
 (let ((?x15 (|(repl st.0)_call| ?x10 ?x12)))
 (let ((?x17 (value_Shared ?x15)))
 (let ((?x53 (granule_data ?x17)))
 (let ((?x93 (select ?x53 gidx.2347733)))
 (let (($x95 (= (g_norm ?x93) zero_granule_data)))
 (let (($x98 (= (e_state_s_granule (select (g_granules (globals ?x17)) gidx.2347733)) 1)))
 (=> $x98 $x95)))))))))))
 ))
 (not $x31308)))
(check-sat)
