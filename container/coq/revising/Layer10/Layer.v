...
Parameter Layer10_init: RData.

Section Layer10_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition Layer10_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition Layer10_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition Layer10_get_flag (f: flag) (st: RData) : bool := false.

  Definition Layer10_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition Layer10_layer :=
    {|
      State := RData;
      Init := Layer10_init;
      Load := load_RData;
      ...
      Alloca := alloc_stack;
      Free := free_stack;
      GetReg :=Layer10_get_reg;
      SetReg := Layer10_set_reg;
      GetFlag := Layer10_get_flag;
      SetFlag := Layer10_set_flag;
      Ptr2Int := ptr_to_int;
      Int2Ptr := int_to_ptr;
      PtrEqb := ptr_eqb;
      PtrLtb := ptr_ltb;
      ...
      PrimCall :=
          ...
          :: ("bitmap_test_and_set", prim bitmap_test_and_set_spec)
          ...
          :: ("timer_output_is_asserted", prim timer_output_is_asserted_spec)
          :: ("va_to_s1tte", prim va_to_s1tte_spec)
          ...
          :: ("validate_rmm_feature_register_0_value", prim validate_rmm_feature_register_0_value_spec)
          :: nil
    |}.
