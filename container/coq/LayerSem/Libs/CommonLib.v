...
Ltac grewrite :=
  (repeat
    let T := fresh "tmp" in
    match goal with
    | [H:?x = ?y |- ?g] => repeat rewrite H; assert(T: True -> x = y) by (intros; apply H); clear H; rename T into H
    end);
  (repeat
    let E := fresh "Eq" in
    match goal with
    | [H: True -> ?x = ?y |- ?g] => assert(E: x = y) by (apply H; constructor); clear H; rename E into H
    end).
...
Ltac eq_ptr :=
  ...
  repeat match goal with
    ...
    | [ |- VPtr _ = VPtr _ ] => f_equal
    end;
  match goal with
  | [ |- ?v1 = ?v2 ] =>
      destruct_ptr v1; destruct_ptr v2;
      simpl in *
  end;
  match goal with
  | [ |- ?v1 = ?v2 ] =>
      rewrite_ptr v1; rewrite_ptr v2;
      f_equal
  end.

Ltac try_solve :=
  repeat rewrite_shift; Z.div_mod_to_equations; try reflexivity; try lia; try nia.
...
Ltac con_semof :=
  tryif
    (match goal with
    | [ |- semof _ [] ?d None ?d' ] => eapply semof_nil_void_intro
    | [ |- semof _ [] ?d (Some (VInt _)) ?d ] => eapply semof_nil_int_pure_intro
    | [ |- semof _ [] ?d (Some (VInt _)) ?d' ] => eapply semof_nil_int_intro
    | [ |- semof _ [(VInt _) :: _] ?d _ ?d' ] => eapply semof_int_cons_intro
    | [ |- semof _ [] ?d (Some (VBool _)) ?d ] => eapply semof_nil_bool_pure_intro
    | [ |- semof _ [] ?d (Some (VBool _)) ?d' ] => eapply semof_nil_bool_intro
    | [ |- semof _ [(VBool _) :: _] ?d _ ?d' ] => eapply semof_bool_cons_intro
    | [ |- semof _ [] ?d (Some (VPtr _)) ?d ] => eapply semof_nil_ptr_pure_intro
    | [ |- semof _ [] ?d (Some (VPtr _)) ?d' ] => eapply semof_nil_ptr_intro
    | [ |- semof _ [(VPtr _) :: _] ?d _ ?d' ] => eapply semof_ptr_cons_intro
    | [ |- semof _ _ _ _ _ ] => econstructor
    end)
  then idtac
  else
  (match goal with
...
Ltac con_temp_env :=
  match goal with
  | [ |- temp_env ] => econstructor
  end.

Ltac con_step :=
  match goal with
  | [ |- exec _ _ (IAlloca _ _ _ _ :: nil)  _ _ _ _ _ ] => eapply exec_alloca
  ...
  | [ |- exec _ _ (ICall ?t _ (?v _) _ :: nil)  _ _ _ _ _ ] =>
      match v with
      | VGlobal =>
          match t with
      ...
      | VLocal =>
          match t with
          | TVoid => eapply exec_call_void_local
          | _ => eapply exec_call_ret_local
          end
      | _ => idtac
      end;
      match goal with
...
Ltac sstep :=
  ...
  repeat multimatch goal with
    | [ H: ?x = ?y |- _ ] => tryif (is_constructor y) then (rewrite H in *) else idtac
  end;
  match goal with
  | [ |- exec _ _ (_ :: nil) _ _ _ _ _ ] => con_step
  | [ |- exec _ _ (_ :: _) _ _ _ _ _ ] => eapply exec_list
  | [ |- semof _ _ _ _ _ ] => con_semof; try reflexivity
  ...
  | [ |- _ /\ _ ] => split
  | _ => idtac
  end; simpl_arith; simpl; try_refl; try eassumption; grewrite; try eassumption.

  Ltac rewrite_equlity :=
    repeat match goal with
    | [H: ?f ?s1 ?ptr1 ?st = Some _ |- ?f ?s1 ?ptr1 ?st = _] => simpl; simpl_arith; try rewrite H; reflexivity
    | [H: ?f ?s1 ?ptr1 ?st ?v = Some _ |- ?f ?s1 ?ptr1 ?st ?v = _] => simpl; simpl_arith; try rewrite H; reflexivity
    | [H: ?f ?a = Some _ |- ?f ?a = _] => rewrite H; reflexivity
    | [H: ?f ?a ?b = Some _ |- ?f ?a ?b = _] => rewrite H; reflexivity
    | [H: ?f ?a ?b ?c = Some _ |- ?f ?a ?b ?c = _] => rewrite H; reflexivity
    | [H: ?f ?a ?b ?c ?d = Some _ |- ?f ?a ?b ?c ?d = _] => rewrite H; reflexivity
    | [H: ?f ?a ?b ?c ?d ?e = Some _ |- ?f ?a ?b ?c ?d ?e = _] => rewrite H; reflexivity
    | [H: ?f ?a ?b ?c ?d ?e ?f = Some _ |- ?f ?a ?b ?c ?d ?e ?f = _] => rewrite H; reflexivity
    | [H: ?x = ?y |- context [ ?x =? ?y ]  ] => rewrite H
    | [H: ?x = ?y |- context [ ?y =? ?x ] ] => rewrite H
    | [H: ?x = ?y |- context [ ?x = ?y ]  ] => rewrite H
    | [H: ?x = ?y |- context [ ?y = ?x ] ] => rewrite H
    end.
    ...
    Ltac sstep2 :=
      ...
      | [ |- out_return _ <> out_break _ -> _ ] => intros
      | [ |- out_return _ <> out_break _ -> _ ] => intros
      | [ |- out_normal <> out_normal -> _] => intros; try contradiction
      | [ |- out_return ?r <> out_return ?r -> _] => intros; try contradiction
      | [ |- out_break ?r <> out_break ?r -> _] => intros; try contradiction
      | [ |- context [ptr_offset _ _] ] => unfold ptr_offset; simpl; simpl_arith
      | [ |- _ /\ _ ] => split
      | _ => idtac
      end; try_refl; simpl_arith; simpl; try_refl; rewrite_equlity; simpl.
  
Ltac vcgen :=
    try lia; (* Eliminate contradict conditions *)
    repeat sstep2; try eauto; (* gextract_mod; *)
    repeat match goal with
        ...
        | [ |- eval _ _ _ _ = Some _ ] => unfold eval; simpl; try reflexivity
    end.
...
Ltac simpl_func Hspec :=
  unfold_spec Hspec; repeat autounfold with sem in Hspec;
  repeat autounfold with spec in Hspec;
  repeat simpl_hyp Hspec;
  repeat match goal with
    | [H: Some _ = Some _ |- _] => inv H
    | [H: _ && _ = true |- _] => apply andb_true_iff in H; destruct H
    | [H: _ =s _ = true |- _] => apply String.eqb_eq in H
    | [H: _ =? _ = true |- _] => apply Z.eqb_eq in H
    | [H: _ <>? _ = true |- _] => apply negb_true_iff in H
    | [H: _ =? _ = false |- _] => apply negb_false_iff in H
    end.
  ...
  repeat (
    ...
    repeat
      ...
      | |- context[if ?C then _ else _] =>
          first[replace C with false by try_solve|replace C with true by try_solve|destruct_if|fail 1]
      end);
    try match goal with
        | [ H: ?x = ?v |- context[?y] ] =>
            let T := fresh "tmp" in
...
Ltac destruct_spec H :=
  ...
  Ltac solve_ueq_output :=
      match goal with
      | |- out_normal = out_break -> _ =>
                       let H := fresh "H" in
                       intro H; inversion H; idtac "on = ob"
    
      | |- out_break = out_normal -> _ => let H := fresh "H" in
                                                 intro H; inversion H; idtac "ob = on"
    
      | |- forall _, out_normal = out_return _ -> _  =>
                                                       let H := fresh "H" in
                                                       intros ? H; inversion H
    
      | |- forall _, out_break = out_return _ -> _  =>
                                                       let H := fresh "H" in
                                                       intros ? H; inversion H
      | _ => idtac
    ...
    Ltac solve_exec_loop_body te te_all Hte' Hnext Hfinal Inv Q R :=
          let T := fresh "T" in
           eexists; eexists; eexists; split_and;
           repeat (repeat sstep2;
             repeat match goal with
                   | [|- context [ eval_list ] ] =>  repeat (intros; try unfold eval_list, eval; try erewrite Hte'; simpl;repeat grewrite; try lia; try reflexivity)
                   | [|- context [ eval ] ] =>
                       repeat (intros; try unfold eval_list, eval; try erewrite Hte'; simpl;repeat grewrite; try lia; try reflexivity)
                   | [|- _ = _] => try solve_equality; try grewrite
               end);
           repeat (match goal with
             ...
             | |- out_break = out_break -> _ => intro;
               ...
               intros; unfold Q; split;
                  ...
                  repeat
                   ...
                   repeat (grewrite; reflexivity) | reflexivity] )
             | |- forall _, out_return _ = out_return _ -> _ => intros
            ...
            repeat
             ...
             end; solve_ueq_output
             ); simpl; try_refl;solve_ueq_output.
             
                                                             
    Ltac solve_Q_break_early Q x10 loop_stop Hnext Hfinal Hte' te te_all :=
                  unfold Q;
                  let T := fresh "T" in
                  let reach := fresh "reach" in
                  let reach_final := fresh "reach_final" in
                   match type of Hfinal with
                   | ?L1 = ?R1 =>
                       match type of Hnext with
                       | ?L2 = ?R2 =>
                           assert(reach: L2 = R2 -> L1 = R2) by (eapply loop_stop with (W := Z.to_nat (512 - 1 - (512 - x10 - 1))) (W1 := Z.to_nat (512 - 1)); lia )
                       end
                   end;
             match type of reach with
             | ?L2 = ?R2 -> ?L1 = ?R2 =>
                 assert (reach_final : L1 = L2) by (pose proof Hnext as D;
               apply reach in D;
               rewrite <- Hnext in D; rewrite D; reflexivity)
             end;
               rewrite reach_final in Hfinal; rewrite Hfinal in Hnext; inv Hnext;
               let ss := fresh "ss" in
                  let vv := fresh "vv" in
                  let Hte := fresh "Hte" in
                  intros; unfold Q; split;
