...
Section GlobalDefs.
  ...
  Definition load_RData_granules (sz: Z) (p: Ptr) (st: RData) : (option (Z * RData)) :=
  ...
  Definition ptr_eqb (p1: Ptr) (p2: Ptr) : bool :=
    (((p1.(pbase)) =? ((p2.(pbase)))) /\ (((p1.(poffset)) =? ((p2.(poffset)))))).
  ...
  Definition store_RData_granules (sz: Z) (p: Ptr) (v: Z) (st: RData) : (option RData) :=
    let idx := ((p.(poffset)) / (16)) in
    let elem_ofs := ((p.(poffset)) mod (16)) in
...
#[global] Hint Unfold store_RData_granules: spec.
Opaque store_RData.
